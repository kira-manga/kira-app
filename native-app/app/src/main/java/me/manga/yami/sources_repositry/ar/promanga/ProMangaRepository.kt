...
class ProMangaRepository @Inject constructor(
...
//            "Smut"
    ...
    override fun fetchChapterDataF(url: String): Flow<State<List<String>>> = flow {
        ...
        Log.d("ProManga3242342341111111111", "Fetching URL: $url")
        try {
    ...
    private fun extractChapterImagesStreaming(
        ...
        try {
            ...
            if (metadata.maps != null && metadata.maps.isNotEmpty()) {
                ...
                Log.e("prooooMAPS11111111", metadata.maps.toString())
                Log.e("prooooMAPSIMAGES1111111", metadata.images.toString())
...
//            } else if (!metadata.images.isNullOrEmpty()) {
//                // Simple images without combining - return full URLs
//                metadata.images.map { imagePath ->
//                    getFullImageUrlst(imagePath, response.cdn_path)
//                }
            } else {
                emptyList()
            }
...
//        val infoUrl = "${apiUrl}api/public/series/$seriesId"
