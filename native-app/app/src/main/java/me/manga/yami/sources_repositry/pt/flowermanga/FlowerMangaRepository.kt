package me.manga.yamiapk.sources_repositry.pt.flowermanga
...
class FlowerMangaRepository @Inject constructor(
    ...
    override suspend fun extractMangaInfo(
        ...
        return MangaInfo(
            ...
            status          = status,
            favoritesCount  = document
                .select("td:contains(Favoritos) + td")
                .text()
                .filter { it.isDigit() },
            chapters        = chapters
        )
        ...
        doc.select("#loop-content .page-item-detail").forEach { card ->
            ...
            val imageUrl = when {
                ...
                img?.attr("srcset")
                    ...
                    ?.split(" ")
                    ?.firstOrNull()
                    ?.let { src -> Jsoup.parse("<img src=\"$src\">", baseUrl).selectFirst("img")?.absUrl("src") }
                    .orEmpty()
            }
