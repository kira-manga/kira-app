package me.manga.yamiapk.sources_repositry.ar.mangamelloplus
...
class MangamelloPlusRepository  @Inject constructor(
    ...
    override fun fetchChapterDataF(url: String): Flow<State<List<String>>> =
        fetchDataWithHeaders({
            ...
            val fullUrl =
                if (normalized.startsWith("http", true)) normalized
    ...
    override fun getChapterImages(json: String): List<String> {
        return try {
            logD { "Parsing chapter images JSON (${json.length} chars)" }
    ...
    override fun parseChapters(json: String): List<ChapterItem> {
        return try {
            logD { "Parsing chapters JSON (${json.length} chars)" }
    ...
    override fun buildItemsImageRequest(
        ...
        logD { "buildItemsImageRequest -> url=$url width=$screenWidthPx" }

        val headers = NetworkHeaders.Builder().apply {
            defaultHeaders.forEach { add(it.key, it.value) }
        }.build()
    ...
    override fun buildImageRequest(
        ...
        logD { "buildImageRequest -> url=$url" }

        val coilHeaders = NetworkHeaders.Builder().apply {
            if (
                url.contains("mangamello", true) ||
                url.contains("mello", true) ||
                url.contains("cdn.mangamello.com", true)
            ) {
                logD { "Applying Mangamello image headers" }
                imgsHeader.forEach { add(it.key, it.value) }
            }
        }.build()
