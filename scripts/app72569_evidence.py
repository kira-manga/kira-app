"""Bounded Loader5 source identities and original Engine physical evidence; never launches Gradle."""
...
SOURCE = "43649a3962871080c9d1442d5cf30a6e6e0c75da"
SOURCE_TREE = "f398cd00ea60161614bd1eb26b636779eb7eaff6"
BRANCH = "remediation/app-7-loader5-android-validation-01"
STATIC_CARRY = dict(status="PASS", source="43649a3962871080c9d1442d5cf30a6e6e0c75da", source_tree="f398cd00ea60161614bd1eb26b636779eb7eaff6", receipt_sha256="45922aa9e215fa89af666d5bd55e5dbc21799f00bab935686a1edc9dec410879")
...
WORK = Path(os.environ["RUNNER_TEMP"]) / "app7-loader5-android-work"
REPORTS = WORK.parent / "app7-loader5-android-reports"
...
def admit():
    require(BRANCH != "UNBOUND" and len(SOURCE) == len(SOURCE_TREE) == 40 and
            all(c in "0123456789abcdef" for c in SOURCE + SOURCE_TREE), "Primary branch/source binding is UNBOUND")
    require(STATIC_CARRY["status"] == "PASS" and STATIC_CARRY["source"] == SOURCE and
            STATIC_CARRY["source_tree"] == SOURCE_TREE and len(STATIC_CARRY["receipt_sha256"]) == 64 and
            all(c in "0123456789abcdef" for c in STATIC_CARRY["receipt_sha256"]), "Primary final-source static carry UNBOUND")
    for key, expected in {"GITHUB_ACTIONS": "true", "GITHUB_EVENT_NAME": "push", "GITHUB_RUN_ATTEMPT": "1",
                          ...
                          "GITHUB_REF": "refs/heads/" + BRANCH}.items():
    ...
    require(git(ROOT, "rev-parse", "HEAD") == os.environ["GITHUB_SHA"] and
            git(ROOT, "rev-parse", "HEAD^@") == SOURCE and
            git(ROOT, "rev-parse", SOURCE + "^{tree}") == SOURCE_TREE,
            "Carrier must be the event commit and a single direct child of the bound source")
...
def write_binding(tests):
    ...
    binding = dict(status="PRIMARY_BOUND_FOR_REVIEW", contract="app7-loader5-android-original-engine",
