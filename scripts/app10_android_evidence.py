...
TEST_PATHS = (":app:testDebugUnitTest",)  # Exactly four classes /29 methods before the APK.
HOST_METHODS = {
    'me.manga.kira.di.AndroidAppVersionBindingTest': (
        'versionBindingReadsTheInstalledReleaseName',
        'versionBindingUsesUnknownWhenPackageVersionIsMissing',
    ),
    'me.manga.kira.core.util.notification.ChapterNotificationHelperTest': (
        'persistsOnlyDiscoveriesThenPostsNewestSixReversedWithOneCover',
        'sharedChapterUrlDiscoveryKeepsCapturedOwnerAcrossDelayedDisplay',
        'appPermissionAndChannelDenialKeepEveryUpdateWithoutCoverOrPost',
        'candidateIdsAndStateCannotRetargetDiscoveryToAnotherManga',
        'duplicateCandidatesAndRepeatedRefreshProduceOnlyOneCommittedRow',
        'notificationWriteFailureRollsBackChaptersWithoutCoverOrStorageRetry',
        'removedAndReaddedParentRejectsOldWorkerSnapshot',
        'notificationServiceFailureDoesNotBlockConstructionOrPersistence',
        'coldHostKoinInstantiationPostsTextWithoutComposingUi',
    ),
    'me.manga.kira.work.LibraryRefreshNotificationTest': (
        'startWorkCancellationJoinsWorkerAndBlockingCoverBeforeAnyLatePost',
        'foregroundChannelSecurityFailureStillCommitsUpdatesAndCompletesStartWork',
        'noNewChaptersRemainsSuccessfulWithoutCoverOrNewUpdates',
        'atomicWorkerInsertErrorIsFailureWithoutRetryOrCover',
        'optionalCoverExpiryOutsideThirtySecondPersistenceBudgetPreservesCommittedSuccess',
    ),
    'me.manga.kira.work.LibraryRefreshWorkTest': (
        'allFailedPartialAndMissingSource_neverStampOrReturnSuccess',
        'libraryAndLocalChapterReadTimeouts_areFailures_notEmptyOrSuccessfulItems',
        'totalTimeout_settlesChildrenAndRetainsConfirmedActiveBatchWork',
        'atomicPersistenceFailure_failsWithoutRetryDisplayOrStamp',
        'lostDiscoveryRace_isSuccess_andOnlyCommittedNotificationsAreCounted',
        'equalCoverWithNoNewChapters_stillRepairsMetadata_butBlankCoverIsSkipped',
        'emptyLibrary_successDoesNotStamp',
        'stampFailure_returnsFailure_despiteCompletedChapterWork',
        'cancellationDuringReadInsertOrStamp_propagates_withoutMetadataRollback',
        'updatesPersistenceRejectionOrItemTimeout_preventsCompletionDisplayAndStamp',
        'totalTimeoutDuringDisplay_retainsObservedAccountingAndSettlesDisplay',
        'cancellationDuringUpdatesPersistenceOrDisplay_propagatesAndSettlesOwnedWork',
        'displayAndProgressFailures_preserveSuccessAndCapturedPayload',
    ),
}
ROOMS = (":data:local:kspAndroidMain",)
COMPILERS = [("app", ":" + m.replace("/", ":") + ":compileAndroidMain") for m in MODULES]
COMPILERS += [("app", p) for p in (":app:compileDebugKotlin", ":app:compileDebugUnitTestKotlin", *ROOMS)]
...
RESOURCES += [":composeApp:generateSourceRemoteConfig", ":app:prepareNotificationAndroidHostRuntime",
              *TEST_PATHS, ":app:packageDebug"]
...
def lineage(rows, compiled):
    ...
    for index, (platform, names) in enumerate(PLATFORMS.items()):
        for build, task, wanted in (("app", ":sources:engine:" + names[0], set(ENGINE_MODULES)),
                                   ("app", TEST_PATHS[0], set(ENGINE_MODULES)),
            ...
            require({r["project"] for r in joins if (r["build"], r["task"], r["platform"]) == (build, task, platform)} == wanted,
                    "Original suppliers must reach the ordinary consumer compilers and the bound app host Test")
    return joins
...
def check_actions(rows):
    ...
    require(all(states.get(("app", p), {}).get("emptyProcessorClasspath") for p in (":app:kspDebugKotlin", ":app:kspDebugUnitTestKotlin")),
...
def retain_xml(rows, device=False):
    ...
    if not device:
        valid &= {(r["class_name"], r["method"]) for r in observed} == {
            (klass, method) for klass, methods in HOST_METHODS.items() for method in methods}
...
def capture_phase(log, device=False):
    require(device and PORTFOLIO["contract"] == "app-native-continuation-app10-v1" and
            PORTFOLIO["tests"] == [] and PORTFOLIO["assembly_root"] is None,
            "This continuation captures only fresh native-device evidence; host/APK receipts stay historical")
    ...
    try:
        ...
        if device:
            ...
            compilers = (":core:compileAndroidMain", ":platform:compileAndroidMain", compiler)
            require(sum(r["build"] == "app" and r["path"] == task and worked(r) for r in completions) == 1 and
                    Counter((r["build"], r["path"]) for r in rows["COMPILE_COMPLETE"]) ==
                    Counter(("app", p) for p in compilers) and all(worked(r) for r in rows["COMPILE_COMPLETE"]),
                    "Fresh ordinary core/platform/device compiler work and connected action required; no cached host/Engine/app batch")
            require(Counter((r["build"], r["task"]) for r in rows["COMPILER_INPUT"]) == Counter(("app", p) for p in compilers) and
                    all(any(r["task"] == p for r in rows["AVIF_CLASS_ORIGINS"]) for p in (":platform:compileAndroidMain", compiler)) and
                    all(any(r["task"] == p for r in rows["AVIF_RESOLUTION"]) for p in (":platform:compileAndroidMain", compiler, task)),
                    "Real fresh dependency/device compiler, class and AAR runtime observations required")
        else:
            check_actions(rows)
