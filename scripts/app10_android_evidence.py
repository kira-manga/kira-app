...
TEST_PATHS = (":data:download:testAndroidHostTest", ":app:testDebugUnitTest")
ROOMS = (":data:local:kspAndroidMain",)
COMPILERS = [("app", ":" + m.replace("/", ":") + ":compileAndroidMain") for m in MODULES]
COMPILERS += [("app", p) for p in (":app:compileDebugKotlin", ":app:compileDebugUnitTestKotlin",
                                    ":data:download:compileAndroidHostTest", *ROOMS)]
...
RESOURCES += [":composeApp:generateSourceRemoteConfig", ":data:download:prepareApp75AndroidHostRuntime",
              ":app:prepareNotificationAndroidHostRuntime", *TEST_PATHS, ":app:packageDebug"]
...
def lineage(rows, compiled):
    ...
    for index, (platform, names) in enumerate(PLATFORMS.items()):
        for build, task, wanted in (("app", ":sources:engine:" + names[0], set(ENGINE_MODULES)),
                                   ("app", TEST_PATHS[1], set(ENGINE_MODULES)),
...
def check_actions(rows):
    ...
    require(Counter((r["build"], r["path"]) for r in completions) == Counter(COMPILERS) and all(worked(r) for r in completions),
            "Every ordinary host/APK Android compiler and original Engine/Room action must work")
    ...
    for label in ("HOST_RUNTIME", "TEST_RUNTIME"):
        require(Counter(r["task"] for r in rows[label]) == Counter(TEST_PATHS), "Both actual before-fork host observations required")
    require(Counter(r["task"] for r in rows["ANDROID_CLASS_ORIGINS"]) == Counter({p: 2 for p in TEST_PATHS}), "Both Android Room/SQLite class joins required")
...
def retain_xml(rows, device=False):
    ...
    return dict(exact_nonzero_no_failures_no_skips=bool(valid), inventory=inventory, observed=observed)
