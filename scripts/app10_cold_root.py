#!/usr/bin/env python3
"""PREPARED / NOT_RUN. Native37/9 continuation; product-identical host29/APK carried, never UI."""
import os
import sys

ADMISSION, BRANCH, SOURCE, SOURCE_TREE = "APP_NEXT_MOBILE_NATIVE9_CONTINUATION_20260915_04", "remediation/app-next-mobile-native9-continuation-20260915-04", "d02540add380ad538c371a8420617ce6d765bbbd", "a99e0f8ebca90d9194c42be479d8b75aaf8f33a8"
MODE = "native-continuation"  # Only native37/9; no host/production-APK replay or UI mode.
...
CARRIED = {
    "app10_cold_root_support.py": "0befe70794b68fb189a3fe1620a766050db40f1102069b676c52a26f17643949",
    "app10-original-engine.init.gradle": "9b87485a97c719c5dd479ac9329c2da6af4f356b2e5313bc0f7c6b364c13a4d4",
    "app29_linux_owned_processes.py": "ac6fb4b61186c09ab06f9b4a543cb58e65dcbcdfe6ba405f6afbe1814b7fc455",
    "app10-android-observer.gradle": "9cc536caa49487ad27065cc751941d630f7b422d69162d89c877906c06f67e25",
    "app10-android-portfolio.json": "de4185db09829828dbe5bfb45602a3b9a3b4d37803a1bf8a8e93091e835c0d04",
    "app10_android_evidence.py": "2237455743f313846e249e8f3bb25ca86c06a82fef65bac8eaadcb3fe2b2cfa4",
}
...
from app10_cold_root_support import (
    ...
    publish_reports, regular, require, sdk_inputs, sha, snapshot, stop_gradle, vacant,
)
...
def start_device(lane, sdk, avdmanager, state):
    ...
    state["phase"] = "device-bootstrap-entry"
    lane.begin_service_bootstrap()  # Closed childless/pipe-empty boundary before either service launch.
...
def write_compile_binding(lane, portfolio, avif, phase):
    ...
    binding = dict(status="PRIMARY_ADMITTED", contract=portfolio["contract"], portfolio_sha256=sha(SCRIPTS / "app10-android-portfolio.json"),
        ...
        tests=portfolio["tests"], assembly_root=portfolio["assembly_root"], device=portfolio["device"],
        phase=phase, max_heap="1g", avif=avif)
...
def gradle_batch(lane, state, inputs, portfolio, avif, device=False):
    require(MODE == "native-continuation" and device and portfolio["tests"] == [] and portfolio["assembly_root"] is None,
            "Only the native continuation batch; no host selectors or production APK assembly")
    phase = "device" if device else "host-apk"
    roots = [portfolio["device"]["task"]] if device else []  # ANDROID_SERIAL selects the owned singleton; AGP9.2.1 --serial mutates an immutable list.
    if not device:
    ...
    state["phase"] = "device-postgradle-drain"
...
def execute(lane, sdk, state, inputs):
    ...
    require(changed == ["A\t" + p for p in CONTROLS], "Carrier must add exactly the eight reviewed native-continuation controls")
    ...
    require(portfolio["contract"] == "app-native-continuation-app10-v1" and portfolio["tests"] == [] and
            portfolio["assembly_root"] is None and portfolio["device"]["dependency_compilation"] == "fresh-core-platform",
            "Native continuation only: fresh core/platform dependencies, no host tests or production APK rebuild")
    carry = portfolio["carry"]["host29_and_production_apk"]
    require(carry["run_id"] == 34952259413 and carry["source"] == ["5195cc4132868c417824875bf7fdf71f76483a7b", "75b725541c183f24f0ed888259a6b8348e1be5d3"] and
            carry["overall_run_status"] == "FAIL" and carry["host_tests"] ==
            {"pass": 29, "fail": 0, "skip": 0, "fresh_in_this_continuation": False},
            "Only the separately verified product-identical host29/APK carry; prior overall FAIL stays")
    ...
    require(shutil.disk_usage(WORK).free >= 8 * 1024**3, "8GiB floor required before native continuation")
    for path in OUTPUTS:
        path.mkdir(mode=0o700)
        state["outputs"][path] = directory_identity(path)
    # Host29 and the ordinary production APK stay historical receipts from run34952259413.
    # No host batch, staged Firebase example, APK download/copy, or Engine/app rebuild here.
    ...
    state["phase"] = "device-bootstrap-seal"
    lane.seal_service_bootstrap([state["server"], state["emulator"]])
    state["phase"] = "device-postboot-drain"
    lane.foreground_drain()  # Natural owned readiness; no UI success/expectation gate.
    # No app install/launch, UI helper or UI prerequisite. AGP runs only the library-native roster.
    ...
    state["phase"] = "device-postreadback-drain"
    lane.foreground_drain()
...
def main():
    ...
    inputs = dict(source=APP_PIN, engine=ENGINE_PIN, carrier=os.environ["GITHUB_SHA"], ref=BRANCH,
                  ...
                  assembly="CARRIED_FROM_34952259413_NOT_REBUILT", host_tests="CARRIED_29_PASS_FROM_34952259413")
    same, restored = False, None
    try:
        ...
        if lane is not None and lane.absent:
            for phase, device in (("device", True),):
                try:
    ...
    validation_ok = state.get("device_completed") and all(state.get("device" + suffix, False)
        for suffix in ("_command_ok", "_stop_ok", "_evidence_ok"))
    ...
    inputs["service_bootstrap"] = lane.service_bootstrap if lane is not None else None
    inputs["service_anchors"] = list(lane.service_cohort.values()) if lane is not None else []
    inputs["phase_failures"] = lane.phase_failures if lane is not None else dict(events=[], omitted=0)
    inputs["phase_stops"] = {"device": state.get("device_stop_ok", False)}
    ...
    scope = "Native continuation: exactly37 authored methods/nine classes; host29 PASS and production APK carried from product-identical run34952259413 (test-only successor), not rerun/rebuilt/downloaded; that run and historical Android09 stay FAIL; no UI, Linux41, fullmatrix, API26/27, other-ABI runtime,16KiB device or release closure"
    return publish_reports(REPORTS, report_id, payloads, dict(source_unchanged=same, cleanup=clean, mode=MODE,
        ui_observed_pass=False, ui_status="NOT_RUN_OWNER_PROHIBITED",
        device_completed=state.get("device_completed", False), scope=scope), passed, errors)
