#!/usr/bin/env python3
"""BOUND diagnostic05 App10 owned KVM diagnostic/full lane; independent result review required."""
import os
import sys

ADMISSION, BRANCH, SOURCE, SOURCE_TREE = "kira-app10-boot-listener-diagnostic-admitted-20260914-05", "remediation/app-10-boot-listener-diagnostic-05", "3fac0a7ba2c98001396e45857a7f6ab9ce1a5eb2", "770300c991be84e679a69a766a4d6ad16a801862"
MODE = "boot-listener-diagnostic"  # Fixed in the reviewed carrier, never selected by arbitrary caller input.
if (ADMISSION == "UNBOUND" or os.environ.get("APP10_COLD_ROOT_ADMISSION") != ADMISSION or len(sys.argv) != 1 or
        MODE not in ("cold-root", "boot-listener-diagnostic") or os.environ.get("APP10_COLD_ROOT_MODE") != MODE):
...
CARRIED = {"app10_cold_root_ui.py": "ad34e4338db656861bb78a4cf964700ef3960def2d88713fec9fec10260f3720",
...
APP_PIN = ("3fac0a7ba2c98001396e45857a7f6ab9ce1a5eb2", "770300c991be84e679a69a766a4d6ad16a801862")
...
def prepare_kvm(lane, state, inputs):
    ...
    receipt = lane.barrier()  # Close every privileged command naturally before SDK/build work.
    require(receipt["absent"] and not receipt["forced"] and not receipt["errors"], "KVM command ownership did not close naturally")
...
def execute(lane, sdk, state, inputs):
    ...
    prepare_kvm(lane, state, inputs)  # Only after the exact source/carrier custody checks above.
    state["phase"] = "sdk"
    inputs["sdk"], avdmanager = sdk_inputs(lane, sdk, runtime_only=MODE == "boot-listener-diagnostic")
    if MODE == "boot-listener-diagnostic":
        # No Gradle, app output/example staging, APK access/install, binding, or UI child.
        require(shutil.disk_usage(WORK).free >= 8 * 1024**3, "8GiB floor required before diagnostic AVD")
...
def main():
    ...
    inputs = dict(source=APP_PIN, engine=ENGINE_PIN, carrier=os.environ["GITHUB_SHA"], ref=BRANCH,
                  ...
                  assembly="NOT_PERMITTED_DIAGNOSTIC" if MODE == "boot-listener-diagnostic" else "NOT_STARTED")
    ...
    try:
        ...
        lane = Lane(WORK, started, work_seconds=600 if MODE == "boot-listener-diagnostic" else 1890)
    ...
    clean &= time.monotonic() <= (lane.cleanup_end if lane is not None else started + 90)
    ...
    diagnostic = state.get("diagnostic_completed", False)
    validation_ok = diagnostic if MODE == "boot-listener-diagnostic" else (
        observed and state.get("assembly_ok") and state.get("immediate_gradle_stop_ok"))
    passed = same and clean and validation_ok and not errors
    if MODE == "cold-root":
        inputs["assembly"] = "SUCCEEDED" if state.get("assembly_ok") else "FAILED_OR_NOT_STARTED"
    ...
    scope = ("Owned API35 boot/listener/wake/menu/teardown diagnostic only; no Gradle, APK, app install, UI proof or closure"
             if MODE == "boot-listener-diagnostic" else
             "Exact-source debug APK / fresh install / new PID+task only; no Android8/static replay, release or closure")
    return publish_reports(REPORTS, report_id, payloads, dict(source_unchanged=same, cleanup=clean, mode=MODE,
        ui_observed_pass=observed, boot_listener_observed_pass=diagnostic, scope=scope), passed, errors)
