...
from app29_linux_owned_processes import OwnedChildren, process_identity, same_identity
...
PUBLIC_CAPS = {**JSON_CAPS, "build.log": 8*MIB, **{name: 8*MIB for name in SHOTS}}
...
def error_row(stage, error):
    return dict(stage=stage, type=type(error).__name__)  # No arbitrary exception/command text.
...
def owned_listener(lane, port):
    ...
    if not inodes:
        return False
    owned = set()
    for identity in lane.owner.track():
    ...
    require(inodes <= owned, "Private service listener is not an owned descendant")
    return True
