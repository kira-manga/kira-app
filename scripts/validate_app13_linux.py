#!/usr/bin/env python3
"""One source-bound App10 Android notification batch; no automatic integration approval."""
import os
import sys

if os.environ.get("APP13_ADMISSION") != "primary-reviewed-app10-android8-linux-01" or len(sys.argv) != 1:
...
def check_cases(portfolio):
    specs = [(evidence.TEST_PATHS[0], "android", "androidJvm", ":composeApp:compileAndroidHostTest", 8, 2)]
    require(len(portfolio["tests"]) == 1 and portfolio["locale_task"] is None, "Only the App10 Android Test root")
    sources, selectors_seen = portfolio["affected"] | portfolio["retained_inputs"], []
    ...
    require(len(set(selectors_seen)) == 8, "No duplicate/cross-root replay")
    fixture = "composeApp/src/androidHostTest/kotlin/me/manga/kira/core/webview/AndroidWebViewComposeTest.kt"
    require(fixture in portfolio["retained_inputs"] and
            all(c["source"] in portfolio["affected"] and "/src/androidHostTest/" in c["source"]
                for c in portfolio["tests"][0]["cases"]), "Only new Android methods; retained fixture is input-only")
...
def read_portfolio():
    ...
    require(portfolio["contract"] == "app10-android8-linux-original-engine" and
            all(re.fullmatch(r"[0-9a-f]{40}", portfolio["source"][k]) for k in ("head", "tree")), "Source UNBOUND")
    affected, retained, statics = portfolio["affected"], portfolio["retained_inputs"], portfolio["statics"]
    require(len(affected) == 8 and retained and not set(affected) & set(retained), "Exact8 affected and disjoint retained inputs required")
    ...
    require(len(kotlin) == 8 and hashlib.sha256(rows).hexdigest() == statics["input_map_sha256"], "Carried Kotlin map does not bind exact affected source")
...
def publish_result(result):
    ...
    record("result.json", dict(status="RESULT_REVIEW_REQUIRED" if eligible else "FAIL", checks=result, errors=ERRORS,
        ...
        limits="16MiB Gradle;8MiB provenance;512KiB other logs;1MiB XML;64KiB other JSON;12 files<=28MiB",
        scope="Original-source Android main/host compilation and8 notification methods only; no Desktop/locale replay, published-byte, PermissionController, process durability, MainActivity cold-root, Native/device/Store/integration credit"))
