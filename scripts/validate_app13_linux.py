...
if os.environ.get("APP13_ADMISSION") != "primary-reviewed-app13-linux-05" or len(sys.argv) != 1:
...
def check_cases(portfolio):
    specs = [(evidence.TEST_PATHS[0], "android", "androidJvm", ":composeApp:compileAndroidHostTest", 1, 1),
    ...
    require(len(set(selectors_seen)) == 9, "No duplicate/cross-root replay")
...
def publish_result(result):
    ...
    record("result.json", dict(status="RESULT_REVIEW_REQUIRED" if eligible else "FAIL", checks=result, errors=ERRORS,
        ...
        limits="16MiB Gradle;8MiB provenance;512KiB other logs;1MiB XML;64KiB other JSON;14 files<=30MiB",
        scope="Original-source Android/Desktop compilation and9 host methods (corrected Android1+unexecuted Desktop8)/locale only; no replay23 or published-byte, Native/device/Store/integration credit"))
