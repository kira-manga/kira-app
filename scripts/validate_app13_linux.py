#!/usr/bin/env python3
"""One source-bound App13 shared-mobile JVM batch; no automatic integration approval."""
import os
import sys

ADMISSION = "primary-reviewed-app13-linux-06"
if ADMISSION.startswith("UNBOUND_") or os.environ.get("APP13_ADMISSION") != ADMISSION or len(sys.argv) != 1:
...
def check_cases(portfolio):
    specs = [(evidence.TEST_PATHS[0], "desktop", "jvm", ":composeApp:compileTestKotlinDesktop", 8, 3)]
    require(len(portfolio["tests"]) == 1 and portfolio["locale_task"] == ":composeApp:checkLocaleKeyParity", "Only the shared-mobile Test and locale roots")
    ...
    require(set(portfolio["retained_test_sources"]) == {sandbox} and
            sum(c["source"] == sandbox for c in portfolio["tests"][0]["cases"]) == 2, "Sandbox2 only once, on the shared-mobile JVM host")
...
def prepare_inputs():
    ...
    (REPORTS / "inputs.log").write_text("Installed configuration inputs hashed; shared-mobile JVM host8 only; no SDK provisioning or executable probe.\n")
...
def publish_result(result):
    ...
    record("result.json", dict(status="RESULT_REVIEW_REQUIRED" if eligible else "FAIL", checks=result, errors=ERRORS,
        ...
        limits="16MiB Gradle;8MiB provenance;512KiB other logs;1MiB XML;64KiB other JSON;13 files<=30MiB",
        scope="Original-source JVM compilation and8 shared-mobile host methods/locale only; no Android/Apple/static replay or published-byte, Native/device/Store/integration credit"))
