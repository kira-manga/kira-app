...
class UiExpectationFailure(RuntimeError):
    """Recognized UI-only miss; transport, deadline and state safety failures are not this class."""
...
def notification_permission_state(raw):
    # AOSP android-15.0.0_r1 Settings.dumpPackageLPr/dumpRuntimePermissionsLPr:
    # only the installed package's User 0 runtime entry establishes this denial.
    permission = "android.permission.POST_NOTIFICATIONS"
    lines = raw.splitlines()

    def section(rows, header, child_indent):
        starts = [i for i, line in enumerate(rows) if re.fullmatch(header, line)]
        body = []
        if len(starts) == 1:
    ...
    titles, packages = section(lines, r"Packages:", 1)
    heads, package = section(packages, rf"  Package \[{re.escape(PKG)}\] \([0-9a-f]+\):", 4)
    users, user = section(package, r"    User 0:.*", 6)
    installed = re.findall(r"(?:^| )installed=(\S+)", package[users[0]]) if len(users) == 1 else []
    runtimes, runtime = section(user, r"      runtime permissions:", 8)
    entries = [line for line in runtime if line.lstrip().startswith(permission)]
    matches = [re.fullmatch(
        rf"        {re.escape(permission)}: granted=(true|false)(?:, flags=\[[^\[\]\r\n]*\])?", line
    ) for line in entries]
    grants = [match.group(1) if match else None for match in matches]
    counts = dict(packages=len(titles), package=len(heads), user_0=len(users), runtime=len(runtimes))
    legacy = re.findall(rf"{re.escape(permission)}: granted=(true|false)", raw)
    excerpt = [(i + 1, line) for i, line in enumerate(lines) if line.lstrip().startswith(permission)]
    return {
        "scope_counts": counts,
        "user_0_installed_field_count": len(installed),
        "user_0_installed": installed[0] if installed in (["true"], ["false"]) else None,
        "runtime_entry_count": len(entries), "runtime_grants": grants[:8],
        "runtime_grants_truncated": len(grants) > 8,
        "legacy_global_match_count": len(legacy), "legacy_global_matches": legacy[:8],
        "legacy_global_matches_truncated": len(legacy) > 8,
        # Bounded permission-only projection, not a complete package dump/private file.
        "permission_line_count": len(excerpt),
        "permission_excerpt": [{"line": number, "text": line[:160]} for number, line in excerpt[:8]],
        "permission_excerpt_truncated": len(excerpt) > 8 or any(len(line) > 160 for _, line in excerpt[:8]),
        "denied": all(count == 1 for count in counts.values()) and installed == ["true"] and grants == ["false"],
    }
...
def main():
    ...
    def wait_for(label, seconds=35):
        ...
        while time.monotonic() < end:
            ...
            time.sleep(0.5)
        require(time.monotonic() < deadline, "Traversal deadline exceeded")
        raise UiExpectationFailure("Expected UI not observed: " + label)
    ...
    def permission_denied():
        result = adb("shell", "dumpsys", "package", PKG)
        check = dict(stage=stage, exit=result.returncode, stdout_bytes=len(result.stdout),
                     stdout_sha256=hashlib.sha256(result.stdout).hexdigest(), stderr_bytes=len(result.stderr))
        report.setdefault("permission_checks", []).append(check)
        check.update(notification_permission_state(result.stdout.decode("utf-8", errors="strict")))
        require(check["denied"], "Expected one explicit denied POST_NOTIFICATIONS runtime permission for installed user 0")
        return True
    ...
    def deny_if_present(items, filename):
        ...
        denial = [n for n in controls if n.get("package") == controller and n.get("resource-id") in {
            controller + ":id/permission_deny_button",
            controller + ":id/permission_deny_and_dont_ask_again_button",
        }]
        if not controls:
            return False
        recognized = (len(denial) == 1 and denial[0].get("enabled") == "true" and
                      denial[0].get("clickable") == "true" and stage in ("welcome_theme", "cold_library") and
                      any(n.get("package") == controller and "notifications" in n.get("text", "").lower()
                          for n in controls))
        if not recognized:
            ...
            try:
                ...
                # Diagnostic safety failures must not be hidden by a continuable UI-only miss.
                report["unexpected_permission_ui"]["screenshot_error"] = type(error).__name__
                raise
            raise UiExpectationFailure("Unexpected system permission UI; do not guess")
        screenshot(filename)
        tap(denial[0])
    ...
    def library(cold):
        ...
        while time.monotonic() < end:
            ...
            time.sleep(0.5)
        require(time.monotonic() < deadline, "Traversal deadline exceeded")
        raise UiExpectationFailure("Actual empty Library not observed")
    ...
    try:
        ...
        report["status"] = "UI_EXPECTATION_FAILED" if type(error) is UiExpectationFailure else "FAIL_OR_BLOCKED"
