...
def prepare_work():
    ...
    record('preexisting-processes.json', process_baseline())
...
def sdk_identity():
    ...
    # Preserve real installed package metadata; AGP resolves the unchanged compileSdk normally.
    ...
    return dict(platform=platform.name, platform_properties=properties,
...
def process_start(entry):
    return (entry / 'stat').read_text().rsplit(')', 1)[1].split()[19]
    ...
    for entry in Path('/proc').iterdir():
        try:
            if entry.name.isdigit() and entry.stat().st_uid == os.getuid():
                baseline[entry.name] = process_start(entry)
        except (FileNotFoundError, ProcessLookupError):
            pass
    return baseline


def marked(entry, baseline):
    if entry.stat().st_uid != os.getuid():
        return False
    # Exact prelaunch PID/start identities are unrelated, even if environ is protected.
    if baseline.get(entry.name) == process_start(entry):
        return False
...
def owned_processes(sig=None):
    ...
    baseline_file = OUT / 'preexisting-processes.json'
