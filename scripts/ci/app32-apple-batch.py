#!/usr/bin/env python3
"""One ordinary Native12 unit-test batch; KGP simulator defaults. App61 input hold remains."""
import json
import os
...
COMPILES = []  # Apple03 already proved the two ComposeApp MAIN targets on this source.
TESTS = {
...
def verify_execution():
    ...
    o.require(sorted(row['path'] for row in completions) == sorted(OBSERVED) and all(
        ...
        'Both Native test compile/link/run closures must actually execute; six exact task completions')
...
def finish(commands, result, result_name='result.json'):
    ...
    result.update(app61_provenance_hold=True, scope='ordinary Native12 unit tests; KGP simulator defaults; no authenticated-input qualification',
