#!/usr/bin/env python3
"""One ordinary Apple2/Native12 diagnostic batch. App61 authenticated-input hold remains."""
import json
import os
...
COMPILES = [':composeApp:compileKotlinIosArm64', ':composeApp:compileKotlinIosSimulatorArm64']
TESTS = {
    ':platform:iosSimulatorArm64Test': [
        'me.manga.kira.platform.download.IosNativeSessionLifetimeTest',
        'me.manga.kira.platform.download.IosNativeSessionInvalidationTest',
        'me.manga.kira.platform.download.IosBackgroundCompletionTest',
        'me.manga.kira.platform.download.IosBackgroundTransportTest.callbackRetainsOutsideTheLiveTreeUntilTheOriginalAttemptAcceptsIt',
        'me.manga.kira.platform.download.IosBackgroundTransportTest.startupPrunesOnlyGeneratedRegularStagesAndPreservesUnknownAndLiveTreeBytes',
        'me.manga.kira.platform.download.IosBackgroundTransportTest.failedStartupPruneRetainsBytesWithoutRetryingOverLaterTransfers'],
    ':data:iosSimulatorArm64Test': [
        'me.manga.kira.data.repository.IosBackgroundOperationExclusionTest',
        'me.manga.kira.data.repository.IosBackgroundEventSettlementTest',
        'me.manga.kira.data.repository.IosBackgroundFailedCleanupTest',
...
def verify_execution():
    ...
    o.record('observed-tests.json', dict(cases=observed, errors=errors))
    ...
    o.require(sorted(row['path'] for row in completions) == sorted(OBSERVED) and all(
        ...
        'Both main compiles and both Native test compile/link/run closures must actually execute')
...
def finish(commands, result):
    ...
    result.update(app61_provenance_hold=True, scope='ordinary Apple2/Native12 only; no authenticated-input qualification',
                  not_proven=['shipping framework/host', 'UI', 'OS-restored URLSession/process relaunch', 'physical device'])
    o.record('result.json', result)
