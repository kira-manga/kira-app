#!/usr/bin/env python3
"""One App26 affected Native data unit-test batch; KGP simulator defaults. App61 input hold remains."""
import json
import os
...
COMPILES = []  # Only the affected data test target; its normal compile/link prerequisites remain.
TESTS = {':data:iosSimulatorArm64Test': ['me.manga.kira.data.repository.IosRestoredDownloadMediaTest',
                                 'me.manga.kira.data.repository.IosRestoredDownloadRetryTest',
                                 'me.manga.kira.data.repository.IosRestoredDownloadSnapshotTest',
                                 'me.manga.kira.data.repository.IosRestoredDownloadSettlementTest',
                                 'me.manga.kira.data.repository.IosManifestRecoverySafetyTest',
                                 'me.manga.kira.data.repository.IosManifestPersistenceTest',
                                 'me.manga.kira.data.repository.IosBackgroundArtifactTest',
                                 'me.manga.kira.data.repository.IosBackgroundOperationExclusionTest.cancelledHostKeepsResolverOperationUntilItsNonCancellableFinallyActuallyFinishes',
                                 'me.manga.kira.data.repository.IosBackgroundOperationExclusionTest.receiverReceiptDoesNotWaitForTheNextChapterNativeHandoff',
                                 'me.manga.kira.data.repository.IosBackgroundOperationExclusionTest.delayedRetryReentersAdmissionBeforeReadingItsFreshRowAndManifest',
                                 'me.manga.kira.data.repository.IosBackgroundEventSettlementTest.pageReceiptWaitsForOriginalTokenRoomProgressWrite',
                                 'me.manga.kira.data.repository.IosBackgroundEventSettlementTest.failureReceiptWaitsForManifestAndOwnedStageButNotForRetryTransfer',
                                 'me.manga.kira.data.repository.IosBackgroundEventSettlementTest.cancelledReceiverScopeDisposesAndAcknowledgesWithoutPublication']}
...
def verify_execution():
    ...
    o.require(not errors and bool(ids) and len(ids) == len(set(ids)) and covered,
    ...
    o.require(sorted(row['path'] for row in completions) == sorted(OBSERVED) and all(
        ...
        'The data Native test compile/link/run closure must execute; three exact task completions')
...
def finish(commands, result, result_name='result.json'):
    ...
    result.update(app61_provenance_hold=True, scope='App26 affected Native data unit tests; KGP simulator defaults; no authenticated-input qualification',
                  not_proven=['shipping framework/host', 'UI', 'OS-restored URLSession/process relaunch', 'physical device'])
    o.record(result_name, result)
    print('App26 Apple batch:', result['status'], '(App61 provenance hold remains)')
