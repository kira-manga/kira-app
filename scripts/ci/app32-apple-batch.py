#!/usr/bin/env python3
"""One App26 Native02 corrected-oracle method rerun; KGP simulator defaults. App61 input hold remains."""
import json
import os
...
TESTS = {':data:iosSimulatorArm64Test': [
    'me.manga.kira.data.repository.IosRestoredDownloadRetryTest.recoveredNativeOwnershipProtectsOnlyItsPageAndNeverStartsAnUnrelatedMissingPage']}
...
def finish(commands, result, result_name='result.json'):
    ...
    result.update(app61_provenance_hold=True, scope='App26 Native02 corrected-oracle single-method rerun; KGP simulator defaults; no authenticated-input qualification',
                  not_proven=['shipping framework/host', 'UI', 'OS-restored URLSession/process relaunch', 'physical device'])
    o.record(result_name, result)
    print('App26 Native02 Apple single-method rerun:', result['status'], '(App61 provenance hold remains)')
