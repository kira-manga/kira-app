#!/usr/bin/env python3
"""One ordinary Apple2 MAIN batch; simulator/Native12 deferred. App61 input hold remains."""
import json
import os
...
TESTS = {}  # Fixed MAIN-only profile. NEXT's Native12 filters remain required in a later batch.
...
def verify_execution():
    ...
    o.require(sorted(row['path'] for row in completions) == sorted(OBSERVED) and all(
        ...
        'Both shipping-target MAIN compiles must actually execute; Native12 remains deferred')
...
def finish(commands, result, result_name='result.json'):
    ...
    result.update(app61_provenance_hold=True, scope='ordinary Apple2 MAIN only; Native12 deferred; no authenticated-input qualification',
