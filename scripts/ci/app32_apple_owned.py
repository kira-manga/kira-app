...
class Commands:
    ...
    def peek(self, job):
        ...
        if info is not None:
            job['waitid'] = dict(pid=info.si_pid, uid=info.si_uid, code=info.si_code, status=info.si_status)
        # Darwin reports siginfo uid=0 even for runner uid501; group() checks the real UID via ps.
        require(info is None or (info.si_pid == job['pid']
