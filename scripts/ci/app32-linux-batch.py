#!/usr/bin/env python3
"""One ordinary App29 Settings/history exact-three JVM gate plus Android/JVM main compilation."""
import json
import os
...
COMPILES = [':composeApp:compileAndroidMain', ':composeApp:compileKotlinDesktop']
# Only the three newly reviewed Settings/history ownership methods; no old-suite replay.
TESTS = {':composeApp:desktopTest': [
    'me.manga.kira.di.ComplaintBackendRequestOwnershipTest.candidateHistoryUsesSameGraphAndRefusesDuplicateOrStaleOpenings',
    'me.manga.kira.di.ComplaintBackendRequestOwnershipTest.retiredRequestHostCannotOpenHistoryOrRetargetCallbacks',
...
def verify_tests(reports):
    ...
    require(not errors and len(ids) == 3 and len(ids) == len(set(ids)) and covered,
            'Require exactly three selected tests, fresh XML, no extras/duplicates/failures/skips')
