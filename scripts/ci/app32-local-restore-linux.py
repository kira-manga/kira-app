...
METHODS = {
    'LocalSourceSelectionRestoreTest': [
        'cold_exact_signed_selection_restores_without_remote_or_commit',
        'missing_selection_cannot_restore_through_a_denied_commit',
        'local_waiter_cancellation_cannot_release_the_refresh_lock',
        'cancelled_local_commit_reconciles_then_rethrows_without_remote',
    ],
    'EffectiveSourceSelectionLifecycleTest': [
        'caller_cancellation_during_required_fetch_preserves_the_warm_catalog_and_floor',
        'cancellation_before_commit_never_publishes_the_private_candidate_and_later_refresh_recovers',
        'cancellation_after_commit_adopts_only_the_matching_verified_durable_candidate',
        'later_winner_prevents_stale_cancellation_publication_and_is_adopted_on_retry',
    ],
    'RetainedSourceSelectionRecoveryTest': [
        'busy_replacement_readopts_current_a_receipt_without_hiding_the_failure',
        'changed_whole_payload_at_same_identity_or_lost_selection_denies_old_a',
        'adoption_error_or_own_timeout_clears_readiness_without_replacing_the_original_error',
        'caller_cancellation_keeps_cleanup_owned_and_never_starts_a_second_reconciliation',
        'ordinary_remote_failure_without_a_pending_replacement_stays_visible_without_recovery',
    ],
}
PACKAGE = 'me.manga.kira.sources.config.'
TESTS = [PACKAGE + cls + '.' + method for cls, methods in METHODS.items() for method in methods]
TASK = ':sources:config:desktopTest'
PROJECTS = (':core', ':domain', ':sources:contracts', ':sources:config')
REQUIRED = [p + ':compileKotlinDesktop' for p in PROJECTS] + [':sources:config:compileTestKotlinDesktop', TASK]
...
CONTROLS = ('.github/workflows/app32-local-restore-linux.yml', 'scripts/ci/app32-local-restore-linux.init.gradle',
            'scripts/ci/app32-local-restore-linux.py')
ROOT = Path.cwd().resolve()
WORK = Path(os.environ['RUNNER_TEMP']) / 'app32-local-restore-work'
OUT = WORK.parent / 'app32-local-restore-evidence'
OUTPUTS = [ROOT / name for name in ('build', '.gradle', '.kotlin')] + [ROOT / m / 'build' for m in MODULES]
MARKER = ('APP32_OWNED_ROOT=' + str(WORK)).encode()
...
def prepare():
    ...
    require(os.environ['GITHUB_EVENT_NAME'] == 'push' and os.environ['GITHUB_REPOSITORY'] == 'kira-manga/kira-app'
            and os.environ['GITHUB_REF'] == 'refs/heads/remediation/app32-local-restore-linux-20260919-01'
...
def run_gradle(env):
    ...
    command += ['--no-daemon', '--no-parallel', '--max-workers=1', '--no-build-cache', '--no-configuration-cache',
                ...
                '-Dorg.gradle.jvmargs=-Xmx3g -XX:MaxMetaspaceSize=1g -XX:ActiveProcessorCount=2',
...
def owned_processes(sig=None):
    """Signal only this fresh invocation's inherited marker, using pidfds against PID reuse."""
    count = 0
    for entry in Path('/proc').iterdir():
        ...
        fd = None
        try:
            if entry.stat().st_uid != os.getuid() or MARKER not in (entry / 'environ').read_bytes().split(b'\0'):
                continue
...
def stop_owned():
    ...
    if (OUT / 'stop.json').is_file():
        ...
        if len(installed) == 1 and (installed[0].parents[2] / 'gradle-9.6.1-bin.zip.ok').is_file():
            ...
            try:
                stopped['exit_code'] = capture(['./gradlew', '--offline', '--stop', '--console=plain'],
...
def cleanup():
    ...
    for path in OUTPUTS + [WORK]:
        ...
        if path.exists():
            ...
            shutil.rmtree(path)
    record('cleanup.json', dict(owned_processes_absent=True, fresh_outputs_absent=all(not p.exists() for p in OUTPUTS + [WORK])))


def verify():
    after = source()
    record('source-after.json', after)
    source_ok = after == json.loads((OUT / 'source-before.json').read_text()) and after['clean']
    ...
    report_root = ROOT / 'sources/config/build/test-results/desktopTest'
    ...
    require(source_ok, 'Source changed')
    require(len(graphs) == 1 and len(graphs[0]) == 38 and len(set(graphs[0])) == 38, 'Normal graph evidence missing')
    require(compilers_ok, 'Five ordinary compilers and Test must actually execute successfully, not be reused/skipped')
