#!/usr/bin/env python3
"""Bound App7 Loader5 Android batch; results always need independent review."""
import os
import sys

ADMISSION = "fd1099944a7e686d7a0245cb81bdea3848257b5c38e0e16d5fd583e497b7fde3"
...
def prepare_inputs():
    ...
    record("inputs.json", dict(installed_input_sha256=inputs, google_prior_absent=True, google_example_sha256=EXAMPLE_SHA,
        ...
        selectors=TESTS, carried_statics=evidence.STATIC_CARRY, gradle_heap="3g", gradle_metaspace="1g", test_heap="1g", work_seconds=1200, stop_seconds=90,
...
def main():
    ...
    record("result.json", dict(status="RESULT_REVIEW_REQUIRED" if eligible else "FAIL", checks=result, errors=ERRORS,
        app_source=evidence.SOURCE, app_source_tree=evidence.SOURCE_TREE, carried_statics=evidence.STATIC_CARRY,
        engine_source=evidence.ENGINE_COMMIT, task=evidence.TEST_PATH, selectors=TESTS,
        limits="16MiB Gradle;8MiB provenance;512KiB other logs;1MiB XML;64KiB other JSON;11 files<=26.8125MiB",
        scope="Original-source Android compilation/Loader5 only; no collector/ownership5/Work/Android57 replay, static rerun, published-byte, ABI/device/Store or integration credit"))
