...
PLATFORMS = {"androidJvm": ("compileAndroidMain", "compileAndroidMain"), "jvm": ("compileKotlinDesktop", "compileKotlinJvm")}
TEST_PATHS = (":composeApp:testAndroidHostTest", ":composeApp:desktopTest")
ROOMS = (":data:local:kspAndroidMain", ":data:local:kspKotlinDesktop")
COMPILERS = [("app", ":" + m.replace("/", ":") + ":" + names[0]) for m in MODULES for names in PLATFORMS.values()]
COMPILERS += [("app", ":composeApp:compileAndroidHostTest"), ("app", ":composeApp:compileTestKotlinDesktop")]
...
OBSERVED += [":composeApp:generateSourceRemoteConfig", ":composeApp:prepareApp13AndroidHostSdk", *TEST_PATHS, ":composeApp:checkLocaleKeyParity"]
...
def admit(portfolio):
    for key, expected in {"GITHUB_ACTIONS": "true", "GITHUB_EVENT_NAME": "push", "GITHUB_RUN_ATTEMPT": "1",
                          ...
                          "GITHUB_REF": "refs/heads/remediation/app-13-public-validation-03"}.items():
...
def write_binding(portfolio):
    ...
    binding = dict(status="PRIMARY_BOUND_FOR_REVIEW", contract=portfolio["contract"], portfolio_sha256=sha(PORTFOLIO_PATH),
        ...
        host_runtime=dict(stage_task=":composeApp:prepareApp13AndroidHostSdk", relative_directory="app13-android-host-runtime/sdk",
                          sdk_configuration="app13HostSdkInput"))
...
def check_actions(rows):
    ...
    require(Counter((r["build"], r["path"]) for r in completions) == Counter(COMPILERS) and all(worked(r) for r in completions),
            "All34 normal compiler actions plus both Room KSP actions must work")
    ...
    require(Counter(r["path"] for r in tasks if r["build"] == "app") == Counter(OBSERVED) and all(worked(r) for r in tasks),
            "Normal resources/SDK/Tests/locale/producer actions must work")
    ...
    require(len(rows["HOST_RUNTIME"]) == 1 and len(rows["ANDROID_CLASS_ORIGINS"]) == 2 and
            Counter(r["task"] for r in rows["TEST_RUNTIME"]) == Counter(TEST_PATHS), "Both before-fork host observations required")
    for room in (r for r in completions if r["build"] == "app" and r["path"] in ROOMS):
        require({"MangaDatabase_Impl", "ChapterDownloadDao_Impl"} <= {Path(r["path"]).stem for r in room["roomSources"]},
                "Real Android and Desktop Room generated implementations required")
    ...
    for row in rows["COMPILE_COMPLETE"]:
        ...
        for output in row["outputs"]:
            ...
            for name, digest in output["neutralClasses"].items():
                ...
                classes[name] = digest
        require(classes, "Fresh compiler classes required for both original modules on both platforms")
...
def check_artifacts(rows):
    ...
    require(len(outgoing) == 1 and outgoing[0]["build"] == "engine", "One actual dual-platform outgoing model required")
