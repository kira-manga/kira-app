"""Bounded App10 source and Android physical evidence; never launches Gradle."""
...
OBSERVED += [":composeApp:generateSourceRemoteConfig", ":composeApp:prepareApp13AndroidHostSdk", *TEST_PATHS]
ROOT = Path(__file__).resolve().parents[1]
ENGINE = ROOT.parent / "engine"
WORK = Path(os.environ["RUNNER_TEMP"]) / "app10-android8-public-work"
REPORTS = WORK.parent / "app10-android8-public-reports"
...
def admit(portfolio):
    for key, expected in {"GITHUB_ACTIONS": "true", "GITHUB_EVENT_NAME": "push", "GITHUB_RUN_ATTEMPT": "1",
                          ...
                          "GITHUB_REF": "refs/heads/remediation/app-10-android8-validation-01"}.items():
...
def check_actions(rows):
    ...
    require(Counter((r["build"], r["path"]) for r in completions) == Counter(COMPILERS) and all(worked(r) for r in completions),
            "All17 normal Android compiler actions plus Android Room KSP must work")
    ...
    require(Counter(r["path"] for r in tasks if r["build"] == "app") == Counter(OBSERVED) and all(worked(r) for r in tasks),
            "Normal resources/SDK/Android Test/producer actions must work")
    ...
    require(len(rows["HOST_RUNTIME"]) == 1 and len(rows["ANDROID_CLASS_ORIGINS"]) == 2 and
            Counter(r["task"] for r in rows["TEST_RUNTIME"]) == Counter(TEST_PATHS), "Exact Android before-fork host observations required")
