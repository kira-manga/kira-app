...
class PageProgressNetworkClientTest {
    @Test
    fun taggedSinkAndFileWritesPreserveExactBytesMetadataAndCloseOwnership() = runTest {
        ...
        try {
            for (toFile in listOf(false, true)) {
                ...
                assertTrue((lastTick.fraction ?: 0f) in 0.99f..1f)
            }
        } finally {
    ...
    @Test
    fun untaggedSameUrlAndUnrelatedRequestsBypassTheBodyDecorator() = runTest {
    ...
    @Test
    fun absentInvalidOrNonPositiveContentLengthStaysIndeterminateAndTimeThrottled() = runTest {
        for (length in listOf(null, "invalid", "0", "-1")) {
            ...
            assertEquals(3, ticks.size, "17 chunks at 10ms must emit at 50ms, 100ms and 150ms")
    ...
    @Test
    fun cancellationPropagatesWithoutASecondPumpOrClosingTheCallersSink() = runTest {
        val entered = CompletableDeferred<Unit>()
        var closed = 0
        val body = object : NetworkResponseBody {
            override suspend fun writeTo(sink: BufferedSink) {
                sink.writeUtf8("partial")
                entered.complete(Unit)
                awaitCancellation()
            }
            override suspend fun writeTo(fileSystem: FileSystem, path: Path) = error("unused file path")
    ...
    private suspend fun assertSinkWrite(body: ProgressBody, output: NetworkResponseBody) {
    ...
    private fun assertResponseMetadata(original: NetworkResponse, wrapped: NetworkResponse) {
    ...
    private fun taggedRequest(statuses: MutableList<PageDownloadProgress>): NetworkRequest = NetworkRequest(
    ...
    private fun response(body: NetworkResponseBody, length: String?): NetworkResponse = NetworkResponse(
        code = 200,
        requestMillis = 123L,
        responseMillis = 456L,
...
private class TrackingSink(val output: Buffer = Buffer()) : ForwardingSink(output) {
    var closes = 0
    var flushes = 0
...
private class ProgressBody(private val beforeChunk: () -> Unit = {}) : NetworkResponseBody {
    val bytes = ByteArray(16 * 1024 + 17) { (it % 251).toByte() }
    var sinkWrites = 0
    var fileWrites = 0
    var closed = 0
    ...
    override suspend fun writeTo(sink: BufferedSink) {
        ...
        while (offset < bytes.size) {
            beforeChunk()
            val count = minOf(1024, bytes.size - offset)
