...
interface SelectionMigrationReads {
    /**
     * Counts the seven materialized ownership tables, including unaffected rows, once each. Locator
     * bytes cover their API/page URL fields; inherited parent APIs are not counted again. The artifact
     * admission scalar probes at most one primary-key row per saved chapter without loading its payload.
    ...
    // One bounded scalar per saved owner; token/path/receipt bytes are not materialized.
    // Null means no artifact row. An existing unknown, non-idle or mismatched owner is false.
    @Query(
        """
        SELECT chapters.id, chapters.mangaId, chapters.url,
          (SELECT CASE WHEN artifacts.mangaId = chapters.mangaId AND artifacts.chapterUrl = chapters.url
              AND artifacts.token IS NULL AND artifacts.operation IS NULL AND artifacts.retiring = 0
              AND artifacts.downloadId IS NULL AND artifacts.pendingRelativePath IS NULL
              AND artifacts.pendingSizeBytes IS NULL AND artifacts.ownsPendingPath = 0
              AND artifacts.retiredRelativePath IS NULL AND artifacts.conversionSourceRoster IS NULL
              AND (artifacts.committedRelativePath IS NULL OR artifacts.committedToken IS NOT NULL)
            THEN 1 ELSE 0 END
           FROM chapter_artifacts AS artifacts WHERE artifacts.chapterId = chapters.id
          ) AS artifactOwnerMovable
        FROM saved_chapters AS chapters ORDER BY chapters.id
        """,
    )
...
/** URL columns only. No conflict-ignore/replacement, lifecycle update, insert, delete or receipt-payload edit. */
interface SelectionMigrationWrites {
    ...
    /** Same exact-owner/idle predicate as the scalar preflight; preserve every non-URL field. */
    @Query(
        """
        UPDATE chapter_artifacts SET chapterUrl = :newUrl
        WHERE chapterId = :id AND mangaId = :mangaId AND chapterUrl = :oldUrl
          AND token IS NULL AND operation IS NULL AND retiring = 0
          AND downloadId IS NULL AND pendingRelativePath IS NULL
          AND pendingSizeBytes IS NULL AND ownsPendingPath = 0
          AND retiredRelativePath IS NULL AND conversionSourceRoster IS NULL
          AND (committedRelativePath IS NULL OR committedToken IS NOT NULL)
        """,
    )
    suspend fun moveChapterArtifact(id: Long, mangaId: Long, oldUrl: String, newUrl: String): Int
...
/** A chapter URL is parent-scoped. Artifact admission is consulted only for an actual URL move. */
data class SelectionSavedChapter(val id: Long, val mangaId: Long, val url: String, val artifactOwnerMovable: Boolean?)
