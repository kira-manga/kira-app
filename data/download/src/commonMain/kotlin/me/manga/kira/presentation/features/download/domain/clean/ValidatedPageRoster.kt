...
internal data class ValidatedPageRoster(val expected: List<Int>, val validated: Map<Int, Path>) {
    val indices: Set<Int> get() = validated.keys
    val completePaths: List<String>?
        get() = if (expected.isNotEmpty() && expected.all { it in validated }) {
            expected.map { validated.getValue(it).toString() }
        } else null
}

internal fun DownloadManifest.hasValidPageRoster(): Boolean =
    pages.isNotEmpty() && pages.all { it.attempts >= 0 } &&
