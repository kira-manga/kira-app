...
class BoundedPageTransferTest {
    ...
    @Test
    fun exactLimitWorksForDeclaredUnknownAndUnderstatedLengths() = runTest {
    ...
    @Test
    fun excessFailsBeforeItCanReplaceAGoodPageAndCleansItsPartial() = runTest {
        fs.createDirectories(directory)
    ...
    @Test
    fun emptyAndStreamFailureCannotPublishAnImage() = runTest {
        fs.createDirectories(directory)
        val temporary = pageTemporaryPath(directory, 0)
    ...
    @Test
    fun cancellationClosesChannelAndDeletesOnlyOwnedPartial() = runTest {
        fs.createDirectories(directory)
        val temporary = pageTemporaryPath(directory, 0)
    ...
    @Test
    fun exclusiveCreateCollisionMustNotDeleteSomeoneElsesPartial() = runTest {
        fs.createDirectories(directory)
        val temporary = pageTemporaryPath(directory, 0)
    ...
    @Test
    fun cleanupFailureDoesNotMaskTheOriginalLimitFailure() = runTest {
        fs.createDirectories(directory)
        val temporary = pageTemporaryPath(directory, 0)
        val failingDelete = object : ForwardingFileSystem(fs) {
            override fun delete(path: Path, mustExist: Boolean) = throw IOException("cleanup failed")
        }
        val failure = assertFailsWith<PageByteLimitExceeded> {
