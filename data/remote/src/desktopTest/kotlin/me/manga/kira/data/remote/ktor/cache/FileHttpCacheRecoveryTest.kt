...
class FileHttpCacheRecoveryTest {
    @Test
    fun oversizedAndNegativeDeclaredLengthsAreRejectedBeforeAnyBodyRead() = runTest {
    ...
    @Test
    fun validRecordUnderWrongIdentityIsDeletedRatherThanServed() = runTest {
        withCacheDirectory { fixture ->
            val policy = smallCachePolicy()
    ...
    @Test
    fun startupRemovesOnlyOwnedStagingAndLegacyFlatCacheFiles() = runTest {
        withCacheDirectory { fixture ->
            val policy = smallCachePolicy()
            ...
            listOf(staging, legacy, unknown).forEach { path ->
                fixture.fileSystem.sink(path).buffer().use { it.writeUtf8("sentinel") }
            }
    ...
    @Test
    fun failedMoveAndCleanupLeaveAtMostOneStagingRecordAndDisableFurtherWrites() = runTest {
        withCacheDirectory { fixture ->
            val policy = smallCachePolicy()
    ...
    @Test
    fun unsafeNamespaceFailsClosedWithoutClearingAnUnrelatedPath() = runTest {
        withCacheDirectory { fixture ->
            fixture.fileSystem.createDirectories(fixture.root)
            val occupied = fixture.root / "public"
            fixture.fileSystem.sink(occupied).buffer().use { it.writeUtf8("not a cache directory") }
            val policy = smallCachePolicy()
            val cache = cacheOwner(policy, fixture.persistence(policy))
            assertEquals(0, cache.snapshot().entries)
            assertFailsWith<IOException> { cache.clear() }
            assertEquals("not a cache directory", fixture.fileSystem.source(occupied).buffer().use { it.readUtf8() })
        }
    }
}

private class PrefixOnlyFileSystem(delegate: FileSystem) : ForwardingFileSystem(delegate) {
    var sourceCalls = 0
    var readBytes = 0L
...
private class FailedStagingFileSystem(delegate: FileSystem) : ForwardingFileSystem(delegate) {
    var fail = true
    var moves = 0
