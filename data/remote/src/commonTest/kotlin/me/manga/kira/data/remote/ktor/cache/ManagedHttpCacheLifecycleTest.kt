...
class ManagedHttpCacheLifecycleTest {
    @Test
    fun publicAndPrivateRecordsWithIdenticalVaryDoNotOverwriteEachOther() = runTest {
        ...
        assertEquals(setOf(CacheNamespace.PRIVATE), disk.records.keys.map { it.namespace }.toSet())
    }

    @Test
    fun findAndRemoveUseExactVaryIdentityNotSubsetMatching() = runTest {
    ...
    @Test
    fun readsPromoteLruAcrossTheSharedNamespaceBudget() = runTest {
        ...
        assertNotNull(cache.publicStorage.find(first.url, emptyMap()))
        assertEquals(setOf(first.url, third.url), disk.records.keys.map { it.url }.toSet())
    }

    @Test
    fun expirySweepsAllResidentBodiesAndTheirExactDiskRecords() = runTest {
    ...
    @Test
    fun clearDropsLiveAndPersistedEntriesAndLaterStoresCanRepopulate() = runTest {
        val disk = RecordingCachePersistence()
        val cache = cacheOwner(persistence = disk)
    ...
    @Test
    fun concurrentStoresReadsAndClearPreserveAllAggregateInvariants() = runTest {
