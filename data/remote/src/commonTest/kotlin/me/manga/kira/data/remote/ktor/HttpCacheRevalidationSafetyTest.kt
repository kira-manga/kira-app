...
class HttpCacheRevalidationSafetyTest {
    @Test
    fun callerAuthoredConditionsAreNeverRemovedOrAutomaticallyRetried() = runTest {
    ...
    @Test
    fun unsolicited304WithoutAnyOwnedValidatorIsNotRetried() = runTest {
    ...
    @Test
    fun getWithNonemptyOutgoingContentIsNeverReplayed() = runTest {
        val cache = cacheOwner()
        var calls = 0
        withHttpCacheClient(cache, { request ->
            calls++
    ...
    @Test
    fun second304PropagatesAtTheRetryCeilingAndBothFailedBodiesAreCancelled() = runTest {
    ...
    @Test
    fun changedValidatorAfterThe304IsNotClobberedByRecovery() = runTest {
        val cache = cacheOwner()
...
private suspend fun TestScope.assertCallerConditionPreserved(name: String, value: String) {
    val cache = cacheOwner()
    var calls = 0
    withHttpCacheClient(cache, { request ->
        if (++calls == 1) {
...
private class RepeatedNotModifiedReplies(scope: TestScope) {
...
private fun changeValidatorAfterCacheMiss() = createClientPlugin("ChangeValidatorAfterCacheMiss") {
    on(Send) { request ->
        try {
            proceed(request)
