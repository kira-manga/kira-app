...
class ManagedHttpCacheBudgetTest {
    ...
    @Test
    fun exactBodyMetadataAndAggregateBoundariesAreAdmitted() = runTest {
        ...
        val policy = smallCachePolicy().copy(
            maxTotalBytes = bytes,
    ...
    @Test
    fun oneByteOverEachBoundaryIsRejectedWithoutRetention() = runTest {
        val data = cachedResponse(body = ByteArray(32))
        val metadata = assertNotNull(HttpCacheMetadataCodec(1_024).encode(data))
        val bytes = metadata.size + data.body.size + CACHE_RECORD_PREFIX_BYTES
        val policies = listOf(
    ...
    @Test
    fun manyUrlsShareOneByteBudgetAcrossBothNamespaces() = runTest {
    ...
    @Test
    fun tinyBodiesStillObeyTheTotalEntryLimit() = runTest {
    ...
    @Test
    fun variantsOfOneUrlShareFourSlotsIncludingBothNamespaces() = runTest {
    ...
    @Test
    fun replacementUpdatesChargeAndCopiesTheCallersBodyAndVary() = runTest {
    ...
    @Test
    fun metadataUtf8ByteCountIncludesLargeHeadersAndVaryValues() = runTest {
    ...
    @Test
    fun lossyUtf8VaryIdentityIsRejectedRatherThanAliasingAnotherRequest() = runTest {
        val cache = cacheOwner()
