...
class HttpCachePluginTest {
    @Test
    fun metadataIsReusedHeadersArePreservedAndActualOwnerClearForcesNetwork() = runTest {
        ...
        withHttpCacheClient(cache, { request ->
            ...
            suspend fun fetch(): String = client.get("https://metadata.test/catalog") {
    ...
    @Test
    fun manyMockResponsesStayWithinTheSharedMetadataBudget() = runTest {
    ...
    @Test
    fun pluginDoesNotRetainHeaderlessNoStoreImageOrEventStreamBodies() = runTest {
        val cases = listOf(
    ...
    @Test
    fun privateAndPublicVaryResponsesCoexistWithoutOverwriting() = runTest {
        val cache = cacheOwner()
        var calls = 0
        withHttpCacheClient(cache, { request ->
            calls++
            ...
            repeat(2) {
                listOf("public", "private").forEach { visibility ->
                    val body = client.get("https://metadata.test/vary") {
    ...
    @Test
    fun downloadStyleUncachedClientHasNoPluginOrOwnerAndNeverRetainsPageGets() = runTest {
        var calls = 0
