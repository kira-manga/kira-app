...
class SourcesLanguageToggleAtomicityTest {
    ...
    fun catalogMembershipIsCapturedOnceBeforeThePersistedRowReadSuspends() =
        runTest {
            val fixture = fixture()
            // Room still says EN: both the accepted language and membership must be captured.
            fixture.catalog.descriptors = listOf(fakeDescriptor("EnglishOne", "(FR)"))
            val gate = EnablementGate().also { fixture.dao.beforeNextRead = it }
            try {
                val command = async { fixture.repository.setLanguageEnabled("(FR)", true) }
                gate.reached.await()
                assertEquals(1, fixture.catalog.snapshotCalls)
                fixture.catalog.descriptors =
                    listOf(fakeDescriptor("EnglishOne", "(EN)"), fakeDescriptor("EnglishTwo", "(FR)"))
                gate.release()
                command.await()
