...
internal class ComplaintHistoryLoads {
    ...
    suspend fun <T> run(load: suspend (ComplaintHistoryWork) -> AppResult<T>): AppResult<T> = coroutineScope {
    ...
    private suspend fun enter(work: ComplaintHistoryWork): Boolean = replacement.withLock {
...
internal class ComplaintHistoryWork(val job: Job, private val loads: ComplaintHistoryLoads) {
...
private sealed interface HistoryLoadState {
    ...
    class Active(val work: ComplaintHistoryWork) : HistoryLoadState
}
