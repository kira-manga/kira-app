...
internal fun deletionLocalError(result: Outcome<*>): AppError = when (result) {
    is Outcome.StorageFailure -> AppError.Storage.Io()
    is Outcome.Invalid -> AppError.Network.Serialization()
    else -> deletionUnavailable()
}

internal fun deletionSessionError(result: ComplaintSessionResult): AppError = when (result) {
...
internal fun deletionTransportError(reason: ComplaintSessionFailure): AppError = when (reason) {
...
internal enum class InstallationDeletionCleanup { NONE, COMPLETED }
