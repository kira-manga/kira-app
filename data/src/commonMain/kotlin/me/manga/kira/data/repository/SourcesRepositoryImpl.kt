 ...
 *
 * **Bulk writes** snapshot active catalog membership before reading persisted rows, select exact
 * case-sensitive language matches, then update every selected name in one SQL statement. Primary
...
class SourcesRepositoryImpl(
    ...
    private suspend fun eligibleSourcesSnapshot(): List<SourcesEntity> {
        val activeApis =
            sourceRegistry
                .genericDescriptors()
                .filter { it.lifecycle == ACTIVE_LIFECYCLE }
                .map { it.api }
                .toSet()
        return sourcesDao.getAllSourcesOnce().filter { it.name in activeApis }
    }

    private suspend fun setSourcesEnabled(
        targets: List<SourcesEntity>,
        enabled: Boolean,
    ) {
        if (targets.isNotEmpty()) {
            sourcesDao.setEnabledByNames(targets.map { it.name }, enabled)
        }
    }

    private companion object {
        const val ACTIVE_LIFECYCLE = "active"
    }
}
