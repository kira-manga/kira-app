...
/** Finite observational edit errors. Only exact status404 OPERATION_NOT_FOUND can qualify a live retry. */
internal enum class ComplaintEditProblem(
    val status: Int,
) {
...
internal object ComplaintEditProblemReader {
    fun read(
        text: String,
        status: Int,
    ): ComplaintEditProblem? {
        ...
        if (root.keys != ROOT_FIELDS || root.historyString("type") != "about:blank" ||
        ...
        val code = ComplaintEditProblem.entries.singleOrNull { it.name == error.historyString("code") }
            ?: invalidHistory()
