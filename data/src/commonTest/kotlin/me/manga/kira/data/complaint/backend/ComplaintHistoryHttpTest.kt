...
class ComplaintHistoryHttpTest {
    @Test
    fun fixedGetAddsOnlyLimitThenOpaqueCursorAndBearerWithoutARequestBody() =
        runTest {
            var calls = 0
            val fixture =
                ComplaintHistoryFixture(this, historyHandler = {
                    ...
                    val response =
                        if (calls == 1) {
                            historyResponse(listOf(historyItem(2)), cursor = "v1.A._")
                        } else {
                            historyResponse(listOf(historyItem(1)))
                        }
            ...
            try {
                ...
                assertEquals(
                    listOf(
                        "$SESSION_BASE_URL/api/v1/complaints?limit=50",
                        "$SESSION_BASE_URL/api/v1/complaints?limit=50&cursor=v1.A._",
                    ),
    ...
    @Test
    fun successCapIsTwoMiBButProblemsRemainSixteenKiBAndEveryBodyCloses() =
        runTest {
            for (status in listOf(HttpStatusCode.OK, HttpStatusCode.NotFound)) {
                ...
                for (over in listOf(0, 1)) {
                    ...
                    val fixture =
                        ComplaintHistoryFixture(
                            this,
                            historyHandler = { respond(channel, status, sessionHeaders(status)) },
                        )
                    try {
    ...
    @Test
    fun invalidHeadersFramingUtf8AndRedirectStatusAreSanitizedAndNeverRetried() =
        runTest {
            ...
            val headers =
                listOf(
                    ...
                    sessionHeaders {
                        append(HttpHeaders.ContentLength, "${body.size}")
            ...
            for (status in listOf(HttpStatusCode.NoContent, HttpStatusCode.Found)) {
                ...
                try {
                    assertIs<AppError.Network.Serialization>(
                        assertIs<AppResult.Failure>(fixture.repository.loadUserComplaints()).error,
                    )
    ...
    @Test
    fun codeLess401NeedsExactChallengeAndMatchingStrictActualProblem() =
        runTest {
            val cases =
                listOf(
                    historyProblem(HttpStatusCode.Unauthorized) to
            ...
            for ((body, headers) in cases) {
                val fixture =
                    ComplaintHistoryFixture(
                        this,
                        historyHandler = { respond(body, HttpStatusCode.Unauthorized, headers) },
                    )
                try {
                    assertIs<AppError.Network.Serialization>(
                        assertIs<AppResult.Failure>(fixture.repository.loadUserComplaints()).error,
    ...
    @Test
    fun unavailableStatusesHaveNoRetryOrEnrollmentFallbackAndTransportTextNeverEscapes() =
        runTest {
            for (status in listOf(
                ...
                val fixture =
                    ComplaintHistoryFixture(
                        this,
                        historyHandler = { respond(historyProblem(status), status, sessionHeaders(status)) },
                    )
                try {
            ...
            val fixture =
                ComplaintHistoryFixture(this, historyHandler = { error("synthetic URL/header/body must not escape") })
            try {
                val error = assertIs<AppResult.Failure>(fixture.repository.loadUserComplaints()).error
    ...
    @Test
    fun ownedHistoryDeadlineIsTypedAndDoesNotRefreshOrClearEvidence() =
        runTest {
    ...
    private suspend fun kotlinx.coroutines.test.TestScope.assertInvalidBody(
        body: ByteArray,
        headers: Headers,
    ) {
...
private suspend fun TestScope.assertCleanHistoryEofRemainsAccepted(status: HttpStatusCode) {
    ...
    val fixture =
        ComplaintHistoryFixture(this, historyHandler = {
            if (++calls == 1) {
                respond(channel, status, sessionHeaders(status))
            } else {
