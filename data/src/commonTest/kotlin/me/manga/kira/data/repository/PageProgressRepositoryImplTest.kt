...
class PageProgressRepositoryImplTest {
    @Test
    fun cancellingNewestProducerRestoresSurvivorWithoutAffectingAnotherReader() = runTest {
    ...
    @Test
    fun returningToSameUrlRejectsOldAttemptsAndUnknownHandles() = runTest {
        val repository = PageProgressRepositoryImpl()
    ...
    @Test
    fun finishedAttemptsAreDiscardedAndCannotOverwriteRetry() = runTest {
        val repository = PageProgressRepositoryImpl()
        val observation = repository.observe("retry")
        val terminalStates = listOf(PageDownloadProgress.Complete, PageDownloadProgress.Failed, PageDownloadProgress.Idle)
        repeat(96) { index ->
    ...
    @Test
    fun repeatedSessionsRetainOnlyCurrentlyAcquiredPagesIncludingUnreportedOnes() = runTest {
        val repository = PageProgressRepositoryImpl()
        repeat(48) { session ->
