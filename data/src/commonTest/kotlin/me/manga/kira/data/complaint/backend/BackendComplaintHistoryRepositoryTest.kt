...
class BackendComplaintHistoryRepositoryTest {
    @Test
    fun retainedPreparedAndDispatchedPendingStillAllowsActualOwnerReadsWithoutWrites() = runTest {
        for (dispatched in listOf(false, true)) {
            ...
            try {
                ...
                assertIs<ComplaintHistory.Backend>(assertIs<AppResult.Success<*>>(fixture.repository.loadUserComplaints()).value)
    ...
    @Test
    fun missingIdentityUsesAcceptedBootstrapDurableEnrollmentThenRealSessionAndHistory() = runTest {
        ...
        try {
            ...
            assertEquals(listOf(InstallationStoreStep.CREATE_BEFORE, InstallationStoreStep.CREATE_STORED), fixture.storage.faults.mutations)
        } finally {
            fixture.close()
        }
    }
    ...
    @Test
    fun session404RetriesOnlyTheSameStoredIdentityWhileRejectionNeverRegenerates() = runTest {
        var calls = 0
        val fixture = ComplaintHistoryFixture(this, sessionHandler = {
            calls++
            if (calls == 1) respond(historyInstallationNotFoundProblem(), HttpStatusCode.NotFound, sessionHeaders(HttpStatusCode.NotFound))
            else respond(sessionResponse(), HttpStatusCode.OK, sessionHeaders())
        })
        try {
            ...
            assertTrue(enrollment.body.toByteArray().decodeToString().contains(Fixtures.ID))
            assertTrue(fixture.enrollment.generator.scopes.isEmpty())
    ...
    @Test
    fun initialSessionAndMissingAdmissionsCannotOutliveLifecycleOrBindingChanges() = runTest {
        for (change in listOf("consent", "reset", "deletion", "identity", "generation", "scope", "pending", "close")) {
    ...
    @Test
    fun checkedCleanupResumesBeforeNewEnrollmentButCorruptOrLockedEvidenceNeverBecomesMissing() = runTest {
        ...
        fixture.enrollment.generator.result = InstallationMaterialGenerationResult.Generated(Fixtures.material(id = Fixtures.OTHER_ID))
        try {
            assertIs<AppResult.Success<*>>(fixture.repository.loadUserComplaints())
            val trace = storage.faults.trace
            assertTrue(trace.indexOf(InstallationStoreStep.MARKER_REMOVED) < trace.indexOf(InstallationStoreStep.CREATE_BEFORE))
            assertEquals(Fixtures.OTHER_ID, assertNotNull(storage.credentials.payloadRecord).material.installationId)
            assertNull(storage.credentials.marker)
        } finally {
            fixture.close()
        }
        val changes: List<(InstallationCoordinatorFixture) -> Unit> = listOf(
            { it.pending.slots += Fixtures.slot(1) },
            { it.pending.slots += sessionPendingSlot(Fixtures.record(generation = 2)) },
            { it.credentials.readFailure = InstallationStorageFailure.TemporarilyUnavailable(InstallationTemporaryFailure.LOCKED) },
        )
        for (change in changes) {
            ...
            try {
                ...
                assertTrue(blocked.sessionRequests.isEmpty() && blocked.historyRequests.isEmpty() && blocked.enrollment.requests.isEmpty())
                assertTrue(blocked.storage.faults.mutations.isEmpty())
                assertTrue(blocked.enrollment.generator.scopes.isEmpty())
            } finally {
                blocked.close()
            }
        }
    ...
    @Test
    fun oneMatching401RefreshRetriesOnlyThatPageAndSecond401StopsTheWholeLoad() = runTest {
        for (secondUnauthorized in listOf(false, true)) {
            var calls = 0
            val fixture = ComplaintHistoryFixture(this, historyHandler = {
                calls++
                when {
                    calls == 1 -> respond(historyResponse(listOf(historyItem(2)), cursor = "v1.next.mac"), HttpStatusCode.OK, sessionHeaders())
                    calls == 2 || secondUnauthorized -> respond(historyProblem(HttpStatusCode.Unauthorized), HttpStatusCode.Unauthorized, sessionHeaders(HttpStatusCode.Unauthorized))
                    else -> respond(historyResponse(listOf(historyItem(1))), HttpStatusCode.OK, sessionHeaders())
                }
            })
            try {
                ...
                if (secondUnauthorized) {
                    ...
                    assertEquals(2, assertIs<ComplaintHistory.Backend>(assertIs<AppResult.Success<*>>(result).value).items.size)
                }
    ...
    @Test
    fun refreshedBindingCannotCombineAlreadyReadPagesWithChangedPendingInventory() = runTest {
        var historyCalls = 0
        var sessionCalls = 0
        val storage = InstallationCoordinatorFixture(Fixtures.record())
        val slot = sessionPendingSlot()
        val fixture = ComplaintHistoryFixture(this, storage = storage, sessionHandler = {
            if (++sessionCalls == 2) storage.pending.slots += slot
            respond(sessionResponse(), HttpStatusCode.OK, sessionHeaders())
        }, historyHandler = {
            historyCalls++
            if (historyCalls == 1) respond(historyResponse(listOf(historyItem(2)), cursor = "v1.next.mac"), HttpStatusCode.OK, sessionHeaders())
            else respond(historyProblem(HttpStatusCode.Unauthorized), HttpStatusCode.Unauthorized, sessionHeaders(HttpStatusCode.Unauthorized))
        })
        try {
    ...
    @Test
    fun stale401CannotEvictNewerLeaseOrReopenClosedSessionOwner() = runTest {
        val fixture = ComplaintHistoryFixture(this)
        try {
    ...
    @Test
    fun sessionSuccessDoesNotAuthorizeLaterDispatchAfterConsentOrDeletion() = runTest {
        for (change in listOf("none", "consent", "deletion")) {
            ...
            try {
                ...
                val load = async {
                    fixture.loads.run { work ->
                        ...
                        try {
                            ...
                            release.await()
                            fixture.storage.coordinator.readHistoryPage(session, fixture.sessions, work, fixture.http, null)
                        } finally {
                ...
                registered.await()
                if (change == "deletion") fixture.storage.coordinator.beginDeletion(permit, Fixtures.KEY).success()
                else if (change == "consent") fixture.storage.coordinator.requestRecovery(RecoveryIntent.Reset(permit)).success()
    ...
    @Test
    fun resetConsentAndDeletionProceedWhileHistoryIoIsPausedAndCancelLatePublication() = runTest {
        for (change in listOf("consent", "reset", "deletion")) {
            ...
            try {
                ...
                if (change == "deletion") fixture.storage.coordinator.beginDeletion(permit, Fixtures.KEY).success()
                    ...
                    if (change == "reset") fixture.storage.coordinator.confirmRecovery(prompt).success()
                    else fixture.storage.coordinator.cancelRecovery(prompt).success()
                }
    ...
    @Test
    fun changedGenerationScopeOrPendingDuringIoRejectsRatherThanPublishingAnyPrefix() = runTest {
    ...
    @Test
    fun replacementClosesPreviousLiveBodyBeforeAdmittingAnotherDecode() = runTest {
    ...
    @Test
    fun closeDuringFinalDurableCheckFencesResultAndRefusesAnotherLoadWithoutClearingStores() = runTest {
...
private suspend fun TestScope.assertDelayedNotFoundCannotEnroll(change: String) {
    ...
    val fixture = ComplaintHistoryFixture(this, sessionHandler = {
        entered.complete(Unit)
        withContext(NonCancellable) { release.await() }
        respond(historyInstallationNotFoundProblem(), HttpStatusCode.NotFound, sessionHeaders(HttpStatusCode.NotFound))
    })
    try {
        val load = async { fixture.repository.loadUserComplaints() }
        entered.await()
        ...
        load.join()
        assertEquals(mutations, fixture.storage.faults.mutations)
        assertTrue(if (observed == null) fixture.storage.credentials.payloadRecord == null else
