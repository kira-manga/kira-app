...
class SourcesRepositoryConfigFilterTest {
    ...
    fun setLanguageEnabledWithFallback_prefersEligiblePrimary() =
        runTest {
            ...
            val registry =
                PilotRegistry(
                    setOf("French", "English"),
                    mapOf("French" to fakeDescriptor("French", "(FR)"), "English" to fakeDescriptor("English", "(EN)")),
                )
        ...
        runTest {
            ...
            val registry =
                PilotRegistry(
                    ...
                    mapOf(
                        "HiddenFrench" to fakeDescriptor("HiddenFrench", "(FR)").copy(lifecycle = "disabled"),
                        "English" to fakeDescriptor("English", "(EN)"),
                    ),
                )
            val impl = backgroundScope.sourceSettingsRepository(dao, registry)
        ...
        runTest {
            ...
            val registry =
                PilotRegistry(
                    ...
                    descriptors + ("Stopped" to fakeDescriptor("Stopped", "(EN)").copy(siteState = "STOPPED")),
    ...
    @Test
    fun exactAcceptedLanguageTagsAndEmptySelectionNeverWidenTheCommand() =
        runTest {
            val rows = listOf(toggleRow("English", "(AR)"))
            val dao = StatefulSourcesDao(rows)
            // Accepted metadata owns language even while the persisted projection is stale.
            val registry = PilotRegistry(setOf("English"), mapOf("English" to fakeDescriptor("English", "(EN)")))
            val impl = backgroundScope.sourceSettingsRepository(dao, registry)
