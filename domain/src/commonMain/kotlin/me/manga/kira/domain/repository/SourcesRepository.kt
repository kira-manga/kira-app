...
interface SourcesRepository {
    ...
    /**
     * Reactive list of accepted content sources in catalog order, joined with persisted user choices.
     * Emits on source-setting changes and accepted catalog publication.
     *
     ...
     *
     * After admission, capture accepted catalog metadata once, then join persisted rows and match
     * the snapshot's exact, case-sensitive language tags. Non-working active sources remain eligible;
     ...
     *
     * **Match rules**: case-sensitive `==` against the accepted catalog language,
     * not a potentially stale row projection. Format is the parenthesised tag the legacy
