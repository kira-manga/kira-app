...
class Commands:
    ...
    def within_cap(self):
        sizes = [path.stat().st_size for task in self.tasks
                 for path in (task['log'], task.get('stderrLog')) if path is not None and path.exists()]
    ...
    def start(self, argv, label, seconds=30, end=None, extra=None, cleaning=False, launch_by=None, split_output=False):
        ...
        require(type(split_output) is bool, 'Split output flag must be boolean')
        ...
        require(not split_output or not leaf, 'Census output cannot be split')
        ...
        if split_output:
            task['stderrLog'] = log.with_name(log.stem + '.stderr.log')
            receipt.update(splitOutput=True, stderrOutput=task['stderrLog'].name)
        ...
        try:
            ...
            with ExitStack() as handles:
                output = handles.enter_context(log.open('xb'))
                error_output = handles.enter_context(task['stderrLog'].open('xb')) if split_output else subprocess.STDOUT
                if split_output:
                    for name, stream in (('outputFileIdentity', output), ('stderrFileIdentity', error_output)):
                        info = os.fstat(stream.fileno())
                        require(stat.S_ISREG(info.st_mode) and info.st_nlink == 1 and info.st_uid == os.geteuid(),
                                'Split output must be a fresh owned regular file')
                        receipt[name] = {'device': info.st_dev, 'inode': info.st_ino, 'uid': info.st_uid}
    ...
    def output(self, task, cleaning=False):
        ...
        if task.get('stderrLog') is not None:
            require(task['stderrLog'].stat().st_size <= 1048576, 'Oversized tool stderr')
    ...
    def wait(self, task, cleaning=False):
        ...
        try:
            ...
            if task.get('stderrLog') is not None and task['stderrLog'].exists():
                receipt['stderrBytesAtLastRead'] = task['stderrLog'].stat().st_size
            self.checkpoint()
