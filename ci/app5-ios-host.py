"""Targeted mobile-owner-delete AppleNative12; no framework, Engine checkout or host/UI execution."""
import hashlib
...
BRANCH = 'candidate/app29-mobile-owner-delete-apple-native12-20260918-01'
...
APP = {'repository': 'kira-manga/kira-app', 'sha': '1b83918087a642b952de22983bb711fc8efd205e', 'tree': '1e8741ebfe32b7365f499fc096f3b385d512da43'}
...
def final_diagnostic(result, scratch):
    try:
        ...
        known = type(count) is int and 0 <= count <= 12
...
def admit():
    ...
    fixed = dict(schema='app-29-apple-native12-v1', status='PRIMARY_BOUND_FOR_REVIEW',
        authorization='APP_29_APPLE_NATIVE12_ONE_ATTEMPT_AUTHORIZED',
...
def native_phase(c):
    try:
        ...
        # One Gradle invocation, only data:remote owner-delete9 plus three affected methods, one owned simulator.
...
def finish(c):
    ...
    result['passed'] = bool(result.get('nativeTestsSucceeded') and result.get('nativeProofPreserved') and result.get('simulatorRemoved')
        and result.get('kotlinTestsExecuted') == 12 and result['sourcePreserved']
...
def host_once(request):
    ...
    c = dict(e=e, owner=owner, h=e, native=native, unique=unique, run=run, roots=roots,
        ...
        runnerHome=home, simulator={'name': 'app29-mobile-owner-delete-apple-native12-' + os.environ['GITHUB_RUN_ID'] + '-1', 'runnerHome': str(home)},
...
def main():
    os.umask(0o077)
    require(sys.argv[1:] in (['request'], ['host']), 'One fixed mobile-owner-delete AppleNative12 phase required')
    request = admit()
    if sys.argv[1] == 'host':
