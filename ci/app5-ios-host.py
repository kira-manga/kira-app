"""Changed shipping Swift/framework build plus targeted Native30; prepared for primary review."""
import hashlib
...
BRANCH = 'remediation/app-next-mobile-host-native30-20260915-01'
...
APP = {'repository': 'kira-manga/kira-app', 'sha': '973924f03ba4800019acd28bff14567bbb91c5a9', 'tree': 'fe1df7f30ad617743bfb896bae5c5b7c2ce4e63d'}
...
def error_diagnostic(stage, error):
    ...
    try:
        ...
        row = {'stage': stage if stage in ('attempt', 'native-tests', 'host-build', 'host-native-evidence', 'entry') else 'unknown',
...
def final_diagnostic(result, scratch):
    try:
        ...
        known = type(count) is int and 0 <= count <= 30
        row = {key: result.get(key) is True for key in (
            'hostBuildSucceeded', 'mainProofPreserved', 'proofPreserved',
...
def admit():
    ...
    fixed = dict(schema='app-seven-apple-v1', status='PRIMARY_BOUND_FOR_REVIEW',
        ...
        xcodegenIntakeAuthorization='PINNED_XCODEGEN_2_46_0_ONE_INTAKE_AUTHORIZED',
        mode='HOST_NATIVE', ref=BRANCH, limits=LIMITS,
...
def native_phase(c):
    try:
        ...
        # One Gradle invocation, three real Native roots, one already-owned simulator.
...
def finish(c):
    ...
    result['passed'] = bool(result.get('hostBuildSucceeded') and result.get('proofPreserved')
        ...
        and result.get('kotlinTestsExecuted') == 30 and result['sourcePreserved']
...
def run_recipe(c):
    ...
    try:
        ...
        c['commands'].call(c['h'].argv(c), 'host-build', seconds=LIMITS['buildSeconds'], end=c['workEnd'])
        c['result']['hostBuildSucceeded'] = True
    except Exception as error:
        error_diagnostic('host-build', error)
    ...
    try:
        ...
        error_diagnostic('host-native-evidence', error)
...
def host_once(request):
    ...
    c = dict(e=e, owner=owner, h=h, gen=gen, products=products, native=native, unique=unique, run=run, roots=roots,
        ...
        'app': APP, 'engine': ENGINE, 'mode': 'HOST_NATIVE', 'task': TASK, 'nativeTasks': list(NATIVE_TASKS), 'carrier': os.environ['GITHUB_SHA'],
...
def main():
    os.umask(0o077)
    require(sys.argv[1:] in (['request'], ['host']), 'One fixed host-plus-Native phase required')
    request = admit()
    if sys.argv[1] == 'host':
