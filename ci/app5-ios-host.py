"""Direct changed framework link plus ReaderShare5 and Room cancellation2; no Xcode host/UI execution."""
import hashlib
...
BRANCH = 'remediation/app-125456-648-apple-framework-native7-20260915-01'
REQUEST = 'ci/app5-ios-host.request.json'
FILES = ('.github/workflows/app5-ios-host.yml', 'ci/app-seven-apple-native.init.gradle', 'ci/app-seven-apple-native.py', 'ci/app-seven-apple.native-tests.json', 'ci/app5-ios-host-commands.py', 'ci/app5-ios-host-evidence.py', 'ci/app5-ios-host-products.py', 'ci/app5-ios-host.init.gradle', 'ci/app5-ios-host.py', 'ci/app5-ios-host.source-pins.json', 'ci/app5-original-engine.init.gradle', 'ci/app8-apple.py')
APP = {'repository': 'kira-manga/kira-app', 'sha': 'e604243c2f3204df2b124cb77133d9ce3e37383c', 'tree': '7cacedffed3084eb31f3c217df416527f0a8e7de'}
...
PLAN = '57e50933746f0afd8a0d126ab9c7c2e6cc79cc73b525cbb772946b069cab7433'
AGREEMENT = '19ad9014c439d784f541eee4f2b1f95638763ede878768e930edb1fea05a8f90'
LIMITS = {'buildSeconds': 1800, 'nativeSeconds': 900, 'uikitSeconds': 0, 'workSeconds': 3000, 'controllerSeconds': 3240, 'cleanupSeconds': 240, 'stopSeconds': 40, 'jobMinutes': 60, 'xcodeWorkers': 0, 'gradleWorkers': 1, 'nativeThreads': 1, 'jvmHeapGiB': 3, 'metaspaceMiB': 768, 'diskFloorGiB': 8, 'hostCommandLogBytes': 8388608, 'nativeCommandLogBytes': 4194304, 'uikitCommandLogBytes': 4194304, 'otherCommandLogBytes': 1048576, 'aggregateCommandLogBytes': 20971520, 'otherAggregateCommandLogBytes': 4194304, 'cleanupCommandLogBytes': 1048576, 'staticFrameworkFingerprintBytes': 1073741824, 'nativeBinaryFingerprintBytes': 1073741824}
TASK = ':composeApp:linkDebugFrameworkIosSimulatorArm64'
NATIVE_TASKS = (':composeApp:iosSimulatorArm64Test', ':data:iosSimulatorArm64Test')
...
def error_diagnostic(stage, error):
    ...
    try:
        ...
        row = {'stage': stage if stage in ('attempt', 'native-tests', 'framework-build', 'framework-native-evidence', 'entry') else 'unknown',
...
def final_diagnostic(result, scratch):
    try:
        ...
        known = type(count) is int and 0 <= count <= 7
        row = {key: result.get(key) is True for key in (
            'frameworkBuildSucceeded', 'mainProofPreserved', 'frameworkProofPreserved',
...
def controls(request):
    require(set(request['controls']) == set(FILES), 'Exactly twelve non-request control hashes required')
...
def admit():
    ...
    fixed = dict(schema='app-125456-apple-v1', status='PRIMARY_BOUND_FOR_REVIEW',
        authorization='APP_125456_APPLE_ONE_ATTEMPT_AUTHORIZED',
        acquisitionAuthorization='PUBLIC_DECLARED_WRAPPER_GRADLE_NATIVE_ONLY',
        mode='FRAMEWORK_NATIVE', ref=BRANCH, limits=LIMITS,
        app=APP, engine=ENGINE, task=TASK, nativeTasks=list(NATIVE_TASKS),
...
def carrier(c):
    ...
    require(sorted(changed) == sorted('A\t' + name for name in (*FILES, REQUEST)), 'Carrier must add only thirteen controls')
...
def collect_framework(c):
    # Snapshot the original framework producer evidence BEFORE Native can change build outputs.
    ...
    if proof is not None:
        ...
        result['frameworkProofPreserved'] = attempt(c, 'actual framework evidence', lambda: c['products'].prove(context, proof)) is not None
    unchanged_inputs(c)
    names = ('app-tasks.json', 'engine-tasks.json', 'proof.json', 'framework-proof.json', 'framework-metadata.json')
    c['frameworkEvidence'] = {name: e.sha(c['reports'] / name) for name in names if (c['reports'] / name).exists()}
    result['frameworkEvidenceBeforeNative'] = dict(c['frameworkEvidence'])
...
def native_phase(c):
    try:
        ...
        # One Gradle invocation, only ComposeApp5 + data2, one already-owned simulator.
...
def collect_and_remove(c):
    ...
    if not c['sourceReady']:
        return
    unchanged_inputs(c)
    require(all(e.sha(c['reports'] / name) == value for name, value in c['frameworkEvidence'].items()),
            'Native invocation changed preserved framework proof')
...
def finish(c):
    ...
    result['passed'] = bool(result.get('frameworkBuildSucceeded') and result.get('frameworkProofPreserved')
        and result.get('nativeTestsSucceeded') and result.get('nativeProofPreserved') and result.get('simulatorRemoved')
        and result.get('kotlinTestsExecuted') == 7 and result['sourcePreserved']
...
def run_recipe(c):
    ...
    try:
        ...
        c['commands'].call(gradle_argv(c), 'host-build', seconds=LIMITS['buildSeconds'], end=c['workEnd'])
        c['result']['frameworkBuildSucceeded'] = True
    except Exception as error:
        error_diagnostic('framework-build', error)
        c['result']['errors'].append('framework build: ' + str(error)[:500])
    try:
        attempt(c, 'immediate stop', lambda: stop(c, 'gradle-stop-immediate'))
        c['result']['afterImmediateStop'] = absence(c)
        if c['result']['afterImmediateStop']['absent'] and c['result'].get('frameworkBuildSucceeded'):
            collect_framework(c)
            # A framework metadata failure remains failure, but need not discard useful Native7 evidence.
            if c['result'].get('mainProofPreserved'):
                native_phase(c)
    except Exception as error:
        error_diagnostic('framework-native-evidence', error)
        c['result']['errors'].append('framework/native evidence: ' + str(error)[:500])
    finally:
        c['end'] = min(c['end'], time.monotonic() + LIMITS['cleanupSeconds'])
        c['commands'].begin_cleanup(c['end'])
        # Dispose only the single owned Native simulator. No UIKit phase is present.
...
def host_once(request):
    ...
    c = dict(e=e, owner=owner, h=products, products=products, native=native, unique=unique, run=run, roots=roots,
        ...
        needsStop=False, stopAttempts=[], nativeExecutables=set(), frameworkEvidence={},
        commands=adapter.commands(owner, run, env, end), result={'status': 'INCOMPLETE', 'passed': False, 'errors': [],
        'app': APP, 'engine': ENGINE, 'mode': 'FRAMEWORK_NATIVE', 'xcodeHostBuilt': False, 'task': TASK, 'nativeTasks': list(NATIVE_TASKS), 'carrier': os.environ['GITHUB_SHA'],
...
def main():
    os.umask(0o077)
    require(sys.argv[1:] in (['request'], ['host']), 'One fixed framework-plus-Native7 phase required')
    request = admit()
    if sys.argv[1] == 'host':
