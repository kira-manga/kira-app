"""Targeted reply+detail AppleNative22 on the exact source; no framework, Engine checkout or host/UI execution."""
import hashlib
...
BRANCH = 'candidate/app29-mobile-reply-detail-apple-native22-20260918-01'
...
APP = {'repository': 'kira-manga/kira-app', 'sha': '394172b8d12167e5a37e108e42f54e9bdb56b62b', 'tree': 'd6d304fbef4c4f93750902a5f9f6b0a7f3168c94'}
...
def final_diagnostic(result, scratch):
    try:
        ...
        known = type(count) is int and 0 <= count <= 22
...
def admit():
    ...
    fixed = dict(schema='app-29-apple-native22-v1', status='PRIMARY_BOUND_FOR_REVIEW',
        authorization='APP_29_APPLE_NATIVE22_ONE_ATTEMPT_AUTHORIZED',
...
def prepare(c):
    ...
    identities['swiftPM'] = e.prepare_swiftpm(roots['app'])
...
def native_phase(c):
    try:
        ...
        # One Gradle invocation, only data:remote reply3 plus engine4 and receive-guard2, one already-owned simulator.
...
def collect_and_remove(c):
    ...
    if not c['sourceReady']:
        return
    unchanged_inputs(c)
    result['swiftPMMetadataOutputsRemoved'] = e.remove_swiftpm_metadata(c)
...
def finish(c):
    ...
    result['passed'] = bool(result.get('nativeTestsSucceeded') and result.get('nativeProofPreserved') and result.get('simulatorRemoved')
        and result.get('kotlinTestsExecuted') == 22 and result['sourcePreserved']
...
def host_once(request):
    ...
    c = dict(e=e, owner=owner, h=e, native=native, unique=unique, run=run, roots=roots,
        ...
        runnerHome=home, simulator={'name': 'app29-mobile-reply-detail-apple-native22-' + os.environ['GITHUB_RUN_ID'] + '-1', 'runnerHome': str(home)},
...
def main():
    os.umask(0o077)
    require(sys.argv[1:] in (['request'], ['host']), 'One fixed reply+detail AppleNative22 phase required')
    request = admit()
    if sys.argv[1] == 'host':
