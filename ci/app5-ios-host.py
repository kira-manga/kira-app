"""Native-only successor: PNG4 + manifest2, ordinary dependencies; agreement-gated."""
import hashlib
...
BRANCH = 'remediation/app-two-batch-native6-20260915-01'
...
APP = {'repository': 'kira-manga/kira-app', 'sha': '63b5f819edeaac3d99b47a05a9a79b482b60aea9', 'tree': 'de0b1c700d27dae0d1eab3351a6eb81584eaeafc'}
...
PLAN = '653ef56a6813a6311b7f82461f3efe99e6aa9db17ecfe064b84e859d1f7db949'
AGREEMENT = '5c11537f08002b67951266c8aa6817af235efdd624a7650676647c93da08ba4f'
LIMITS = {'buildSeconds': 0, 'nativeSeconds': 1200, 'uikitSeconds': 0, 'workSeconds': 1620, 'controllerSeconds': 1860, 'cleanupSeconds': 240, 'stopSeconds': 40, 'jobMinutes': 40, 'xcodeWorkers': 1, 'gradleWorkers': 1, 'nativeThreads': 1, 'jvmHeapGiB': 3, 'metaspaceMiB': 768, 'diskFloorGiB': 8, 'hostCommandLogBytes': 8388608, 'nativeCommandLogBytes': 4194304, 'uikitCommandLogBytes': 4194304, 'otherCommandLogBytes': 1048576, 'aggregateCommandLogBytes': 20971520, 'otherAggregateCommandLogBytes': 4194304, 'cleanupCommandLogBytes': 1048576, 'staticFrameworkFingerprintBytes': 1073741824, 'nativeBinaryFingerprintBytes': 1073741824}
NATIVE_TASKS = (':platform:iosSimulatorArm64Test', ':data:download:iosSimulatorArm64Test')
...
def admit():
    ...
    fixed = dict(schema='app-seven-apple-v1', status='PRIMARY_BOUND_FOR_REVIEW',
        ...
        app=APP, engine=ENGINE, nativeTasks=list(NATIVE_TASKS),
...
def native_phase(c):
    try:
        ...
        # One Gradle invocation, two real Native roots, one already-owned simulator.
...
def finish(c):
    ...
    result['passed'] = bool(result.get('nativeTestsSucceeded') and result.get('nativeProofPreserved') and result.get('simulatorRemoved')
        and result.get('kotlinTestsExecuted') == 6 and result['sourcePreserved']
...
def native_once(request):
    ...
    c = dict(e=e, owner=owner, h=h, native=native, unique=unique, run=run, roots=roots,
        ...
        'app': APP, 'engine': ENGINE, 'mode': 'NATIVE_ONLY', 'nativeTasks': list(NATIVE_TASKS), 'carrier': os.environ['GITHUB_SHA'],
