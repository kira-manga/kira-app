"""Targeted mobile-edit AppleNative13 on the exact source; no framework, Engine checkout or host/UI execution."""
import hashlib
...
BRANCH = 'candidate/app29-mobile-edit-apple-native13-20260918-01'
...
APP = {'repository': 'kira-manga/kira-app', 'sha': '448a014dacbff124347706cd3944a0aa473465d1', 'tree': 'c7027e371f2b5672f3ea9d27dec2685f8b6a6050'}
...
def final_diagnostic(result, scratch):
    try:
        ...
        known = type(count) is int and 0 <= count <= 13
...
def admit():
    ...
    fixed = dict(schema='app-29-apple-native13-v1', status='PRIMARY_BOUND_FOR_REVIEW',
        authorization='APP_29_APPLE_NATIVE13_ONE_ATTEMPT_AUTHORIZED',
...
def native_phase(c):
    try:
        ...
        # One Gradle invocation, only data:remote edit9 plus four affected methods, one already-owned simulator.
...
def finish(c):
    ...
    result['passed'] = bool(result.get('nativeTestsSucceeded') and result.get('nativeProofPreserved') and result.get('simulatorRemoved')
        and result.get('kotlinTestsExecuted') == 13 and result['sourcePreserved']
...
def host_once(request):
    ...
    c = dict(e=e, owner=owner, h=e, native=native, unique=unique, run=run, roots=roots,
        ...
        runnerHome=home, simulator={'name': 'app29-mobile-edit-apple-native13-' + os.environ['GITHUB_RUN_ID'] + '-1', 'runnerHome': str(home)},
...
def main():
    os.umask(0o077)
    require(sys.argv[1:] in (['request'], ['host']), 'One fixed mobile-edit AppleNative13 phase required')
    request = admit()
    if sys.argv[1] == 'host':
