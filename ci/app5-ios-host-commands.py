"""Reused finite-work-log adapter; its legacy UIKit allowance is unreachable in this carrier."""
...
def commands(owner, run, env, end):
    class HostCommands(owner.Commands):
        ...
        def output(self, task, cleaning=False):
            ...
            argv0 = (str(Path(self.env['KIRA_APP5_CONTROL']).parent / 'app/gradlew') if receipt['label'] in ('host-build', 'native-tests')
