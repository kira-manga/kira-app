"""One finite link log plus split canary/query logs; donor ownership/settlement reused."""
...
def host_within_cap(self):
    ...
    for index, task in enumerate(self.tasks):
        ...
        if task.get('stderrLog') is not None:
            stderr_size = max(task.get('stderrBudgetBytesSeen', 0), receipt.get('stderrBytesAtLastRead', 0))
            for path, key in ((task['log'], 'outputFileIdentity'), (task['stderrLog'], 'stderrFileIdentity')):
                try:
                    part = path.lstat()
                except FileNotFoundError:
                    custody = False
                ...
                custody &= (stat.S_ISREG(part.st_mode) and part.st_nlink == 1
                            and (part.st_dev, part.st_ino, part.st_uid) ==
                            (bound.get('device'), bound.get('inode'), bound.get('uid')))
                if key == 'stderrFileIdentity':
                    stderr_size = max(stderr_size, part.st_size)
            task['stderrBudgetBytesSeen'] = stderr_size
            # Each descriptor and the combined command consume the same existing allowance.
            custody &= receipt.get('splitOutput') is True and not task['leaf']
            size += stderr_size
        is_cleanup = self.cleanup_at is not None and index >= self.cleanup_at
        if receipt['label'] == 'native-tests':
    ...
    self.log_custody_failed |= not custody or len(native) > 1
    ...
    normal_ok = not self.log_custody_failed and native_ok and work_ok and cleanup_ok
    self.log_budget_failed |= not normal_ok
    self.log_budget = dict(nativeBytes=sum(native), workOtherBytes=sum(work), cleanupBytes=sum(cleanup),
        cleanupReserveBytes=CLEANUP_LOG_RESERVE, cleanupEntered=self.cleanup_at is not None,
        nativeOverflow=not native_ok,
    ...
    return (not self.log_custody_failed and work_ok and cleanup_ok
            and (self.cleanup_at is not None or (native_ok and not self.log_budget_failed)))
...
def commands(owner, run, env, end):
    class HostCommands(owner.Commands):
        ...
        def start(self, argv, label, seconds=30, end=None, extra=None, cleaning=False, launch_by=None, split_output=False):
            owner.require(label not in ('host-build', 'uikit-tests'), 'No app/Swift/UIKit work in App70')
            owner.require(not split_output or label in ('apple-writer-canary', 'apple-writer-unified'),
                          'Only the two fixed App70 raw captures may split output')
            ...
            if self.cleanup_at is not None:
                ...
                owner.require(label != 'native-tests' and self.within_cap(), 'Reserved cleanup diagnostic budget exceeded')
            return super().start(argv, label, seconds=seconds, end=end, extra=extra,
                                 cleaning=cleaning, launch_by=launch_by, split_output=split_output)

        def output(self, task, cleaning=False):
            owner.require(self.within_cap(), 'Host work/cleanup diagnostic budget exceeded at join')
            if task['receipt']['label'] != 'native-tests':
                return super().output(task, cleaning=cleaning)
            receipt = task['receipt']
            cap = 4194304
            argv0 = str(Path(self.env['KIRA_APP5_CONTROL']).parent / 'app/gradlew')
