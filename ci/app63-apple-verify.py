"""Apple06 helper narrowed to pending Native27 and one normal Kotlin iOS compile."""
...
TASK_MODULES = {':data:download:iosSimulatorArm64Test': 'data/download', ':data:iosSimulatorArm64Test': 'data'}
...
def selection():
    ...
    assert selected['compile_task'] == ':composeApp:compileKotlinIosSimulatorArm64'
    identities = [(TASK_MODULES[row['task']], row['class'], method)
                  for row in selected['suites'] for method in row['methods']]
    assert len(identities) == len(set(identities)) == selected['native_cases'] == 27
    assert Counter(identity[0] for identity in identities) == selected['native_module_counts'] == {'data/download': 5, 'data': 22}
    for row in selected['suites']:
        ...
        if row['task'] == ':data:iosSimulatorArm64Test':
            assert declared == row['methods']
...
def native():
    ...
    for module in ('data/download', 'data'):
    ...
    save('native-proof.json', {'status': 'PASS' if qualified else 'FAIL', 'expected_cases': 27,
        ...
        'scope': 'Native data/download5 and data22: exact prior19 plus strict manifest8; no prior platform34 replay, Swift host, physical device, OS wake or SQL migration claim'})
