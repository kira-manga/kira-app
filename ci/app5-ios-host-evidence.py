...
JSON_CAPS.update({'prerequisites.json': 131072, 'project.json': 262144, 'host-proof.json': 1048576,
                  'swiftpm.json': 1048576, 'build-result.json': 131072})
...
def inputs(root, role, pins):
    ...
    if role == 'app':
        ...
        require(files == expected_host and len(files) == 41 and
                len([n for n in files if n.endswith('.swift')]) == 14, 'Shipping host input set changed')
        result['host'] = files
    return result
...
def preserve_log(task, reports, label, result, end):
    ...
    failure_prefix = label == 'host-build' and not result.get('hostBuildSucceeded') and raw_size > cap
    ...
    if failure_prefix:
        result['hostFailureLogPrefix'] = {'truncated': True, 'rawBytes': raw_size, 'retainedBytes': len(data),
...
def retain(commands, reports, result):
    ...
    if result.get('hostBuildSucceeded') and result.get('proofPreserved'):
        require(set(present) == set(PUBLIC_CAPS), 'Missing required bounded host evidence')
