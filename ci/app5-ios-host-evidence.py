"""App5 linked-host evidence; fixed source/Native boundaries, no replay or tool launch."""
import hashlib
import json
import os
...
OUTPUTS = {
    'app': ('build', '.gradle', '.kotlin') + tuple(p + '/build' for p in (*MODULES, 'app', 'desktopApp')) +
        ('iosApp/iosApp.xcodeproj', 'iosApp/.swiftpm', 'iosApp/KotlinMultiplatformLinkedPackage',
         'iosApp/reader-controls-tests/ReaderChromeControlsTests.xcodeproj'),
...
JSON_CAPS.update({'prerequisites.json': 131072, 'project.json': 262144, 'host-proof.json': 1048576,
                  'swiftpm.json': 1048576, 'build-result.json': 131072, 'framework-metadata.json': 65536})
LOGS = ('host-build', 'gradle-stop-immediate', 'gradle-stop-final', 'framework-arch',
        'host-binary-arch', 'host-binary-platform', 'host-linked-binary-arch', 'host-linked-binary-platform')
PUBLIC_CAPS = dict(JSON_CAPS, **{name + '.log': 1048576 for name in LOGS})
PUBLIC_CAPS.update({'host-build.log': 8388608, 'ComposeApp.h': 4194304,
                   'ComposeApp.modulemap': 65536, 'generated-project.patch': 262144})
HOST_REQUIRED = frozenset(PUBLIC_CAPS)
...
NATIVE_LOGS = ('native-tests', 'gradle-stop-native') + tuple(
    'native-' + module + '-' + kind for module in ('platform', 'data-download', 'composeApp') for kind in ('arch', 'platform'))
NATIVE_XML = (
    'native-xml/platform/TEST-iosSimulatorArm64Test.me.manga.kira.platform.media.IosPngIntegrityTest.xml',
    'native-xml/platform/TEST-iosSimulatorArm64Test.me.manga.kira.platform.notification.IosDownloadNotifierContentTest.xml',
    'native-xml/data/download/TEST-iosSimulatorArm64Test.me.manga.kira.presentation.features.download.domain.clean.DownloadManifestStoreTest.xml',
    'native-xml/composeApp/TEST-iosSimulatorArm64Test.me.manga.kira.di.IosSourceActivationBridgeTest.xml',
    'native-xml/composeApp/TEST-iosSimulatorArm64Test.me.manga.kira.reader.ReaderNativeLifecycleTest.xml',
    'native-xml/composeApp/TEST-iosSimulatorArm64Test.me.manga.kira.reader.ReaderNativeEffectDispatchTest.xml',
)
JSON_CAPS.update(NATIVE_JSON_CAPS)
PUBLIC_CAPS.update(NATIVE_JSON_CAPS)
PUBLIC_CAPS.update({name + '.log': 1048576 for name in (*NATIVE_LOGS, 'gradle-stop-cleanup')})
PUBLIC_CAPS.update({'native-tests.log': 4194304, **{name: 1048576 for name in NATIVE_XML}})
LOGS += NATIVE_LOGS + ('gradle-stop-cleanup',)
NATIVE_REQUIRED = (set(NATIVE_JSON_CAPS) - {'native-engine-tasks.json'}) | set(NATIVE_XML) | {name + '.log' for name in NATIVE_LOGS}
UIKIT_JSON_CAPS = {'uikit-source.json': 65536, 'uikit-project.json': 131072, 'uikit-results.json': 131072,
    'uikit-proof.json': 1048576, 'uikit-xcresult-summary.json': 131072, 'uikit-xcresult-tests.json': 262144,
    'uikit-xcresult-summary-schema.json': 65536, 'uikit-xcresult-tests-schema.json': 65536}
UIKIT_LOGS = ('uikit-tests', 'uikit-binary-arch', 'uikit-binary-platform', 'uikit-host-arch', 'uikit-host-platform')
JSON_CAPS.update(UIKIT_JSON_CAPS)
PUBLIC_CAPS.update(UIKIT_JSON_CAPS)
PUBLIC_CAPS.update({name + '.log': 1048576 for name in UIKIT_LOGS})
PUBLIC_CAPS.update({'uikit-tests.log': 4194304, 'uikit-Package.resolved': 262144})
LOGS += UIKIT_LOGS
UIKIT_REQUIRED = set(UIKIT_JSON_CAPS) | {name + '.log' for name in UIKIT_LOGS} | {'uikit-Package.resolved'}

# This fixed successor restores the changed shipping host build, but never UIKit tests.
# Admit only host/Native evidence and twelve explicitly named unexpected XML slots.
JSON_CAPS['native-xml-diagnostics.json'] = 131072
NATIVE_BASE = {'request.json', 'result.json', 'identity.json', 'source.json', 'tools.json', 'commands.json'}
LOGS = ('host-build', 'gradle-stop-immediate', 'gradle-stop-final', 'framework-arch',
        'host-binary-arch', 'host-binary-platform', 'host-linked-binary-arch', 'host-linked-binary-platform') + NATIVE_LOGS + ('gradle-stop-cleanup',)
COMBINED_PUBLIC = HOST_REQUIRED | NATIVE_BASE | set(NATIVE_JSON_CAPS) | set(NATIVE_XML) | {name + '.log' for name in LOGS}
PUBLIC_CAPS = {name: cap for name, cap in PUBLIC_CAPS.items() if name in COMBINED_PUBLIC}
PUBLIC_CAPS['native-xml-diagnostics.json'] = 131072
PUBLIC_CAPS.update({'native-xml-unexpected/' + module + '/' + str(index).zfill(2) + '.xml': 1048576
                   for module in ('platform', 'data/download', 'composeApp') for index in range(1, 5)})
NATIVE_REQUIRED |= NATIVE_BASE | {'native-xml-diagnostics.json', 'gradle-stop-final.log'}
...
def prove(run, roots, end):
    ...
    return {'status': 'SOURCE_AND_HOST_EVIDENCE_REVIEW_REQUIRED', 'task': TASK, 'mainCount': 16,
            'outputs': outputs, 'native': native_markers(run, proofs, end),
            'scope': 'Normal16 main producers within one shipping simulator framework/host build; zero tests/runtime'}
...
def preserve_log(task, reports, label, result, end):
    ...
    phase_success = {'host-build': 'hostBuildSucceeded', 'native-tests': 'nativeTestsSucceeded', 'uikit-tests': 'uikitTestsSucceeded'}
...
def retain(commands, reports, result):
    ...
    for name, cap in PUBLIC_CAPS.items():
        ...
        if path.exists():
            ...
            present.append(name)
    if result.get('hostBuildSucceeded') and result.get('proofPreserved'):
        require(HOST_REQUIRED <= set(present), 'Missing required bounded host evidence')
    ...
    if result.get('uikitTestsSucceeded') and result.get('uikitProofPreserved'):
        require(UIKIT_REQUIRED <= set(present), 'Missing required bounded UIKit evidence')
