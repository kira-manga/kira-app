...
NATIVE_LOGS = ('native-tests', 'gradle-stop-native') + tuple(
    'native-' + module + '-' + kind for module in ('platform', 'data-download') for kind in ('arch', 'platform'))
NATIVE_XML = (
...
# This fixed successor invokes only Native. Keep the established helpers, but
# admit no host/UIKit artifacts and only eight explicitly named unexpected XML slots.
...
NATIVE_PUBLIC = NATIVE_BASE | set(NATIVE_JSON_CAPS) | set(NATIVE_XML) | {name + '.log' for name in LOGS}
PUBLIC_CAPS = {name: cap for name, cap in PUBLIC_CAPS.items() if name in NATIVE_PUBLIC}
...
PUBLIC_CAPS.update({'native-xml-unexpected/' + module + '/' + str(index).zfill(2) + '.xml': 1048576
                   for module in ('platform', 'data/download') for index in range(1, 5)})
