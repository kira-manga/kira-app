...
PUBLIC_CAPS.update({'host-build.log': 8388608, 'ComposeApp.h': 1048576,
...
NATIVE_LOGS = ('native-tests', 'gradle-stop-native') + tuple(
    'native-' + module + '-' + kind for module in ('composeApp', 'data', 'presentation') for kind in ('arch', 'platform'))
NATIVE_XML = (
    'native-xml/composeApp/TEST-iosSimulatorArm64Test.me.manga.kira.reader.ReaderNativeFeedProjectionTest.xml',
    'native-xml/data/TEST-iosSimulatorArm64Test.me.manga.kira.data.repository.PageProgressRepositoryImplTest.xml',
    'native-xml/presentation/TEST-iosSimulatorArm64Test.me.manga.kira.presentation.reader.ReaderPageProgressOwnershipTest.xml',
)
...
def inputs(root, role, pins):
    ...
    if role == 'app':
        ...
        require(files == expected_host and len(files) == 42 and
                len([n for n in files if n.endswith('.swift')]) == 15, 'Shipping host input set changed')
        result['host'] = files
    return result
