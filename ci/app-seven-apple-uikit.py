...
CLASSES = ('ReaderImageDecodeTests',)
COUNTS = (7,)
...
def source_inputs(c):
    ...
    require(set(pins) == {'schema', 'status', 'app', 'target', 'caseCount', 'suites', 'testSwiftInputs', 'inputs'}
            ...
            and type(pins['caseCount']) is int and pins['caseCount'] == 7
            and {'target': TARGET, 'classes': list(CLASSES), 'caseCount': pins['caseCount']} == c['request']['uikitTests'],
            'UIKit roster remains UNBOUND')
    suites = pins['suites']
    require(isinstance(suites, list) and len(suites) == len(CLASSES), 'Exactly one targeted UIKit suite')
    ...
    require(isinstance(expected, dict) and set(expected) == set(pins['testSwiftInputs']) |
            ...
            and len(pins['testSwiftInputs']) == len(set(pins['testSwiftInputs'])) == 26
