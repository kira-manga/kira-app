...
TARGET, HOST = 'ReaderChromeControlsTests', 'ReaderChromeControlsHost'
CLASSES = ('ReaderImageOwnershipTests', 'ReaderImageLoaderBehaviorTests', 'ReaderImageDecodeTests',
           'ReaderPageTransportTests', 'ReaderPageCellOwnershipTests', 'WebtoonPrefetchOwnershipTests',
           'WebtoonReaderBoundaryTests')
COUNTS = (4, 6, 3, 7, 6, 6, 6)
...
def source_inputs(c):
    ...
    require(set(pins) == {'schema', 'status', 'app', 'target', 'caseCount', 'suites', 'testSwiftInputs', 'inputs'}
            and pins['schema'] == 'app-seven-apple-uikit-tests-v1' and pins['status'] == 'PRIMARY_BOUND_FOR_REVIEW'
            and pins['app'] == c['request']['app'] and pins['target'] == TARGET
            and type(pins['caseCount']) is int and pins['caseCount'] == 38
            and {'target': TARGET, 'classes': list(CLASSES), 'caseCount': pins['caseCount']} == c['request']['uikitTests'],
            'UIKit roster remains UNBOUND')
    suites = pins['suites']
    require(isinstance(suites, list) and len(suites) == len(CLASSES), 'Exactly seven UIKit suites')
    for spec, cls, count in zip(suites, CLASSES, COUNTS):
        require(set(spec) == {'class', 'methods'} and spec['class'] == cls
                and isinstance(spec['methods'], list) and len(spec['methods']) == len(set(spec['methods'])) == count
                and all(re.fullmatch(r'test[A-Za-z0-9_]+', name) for name in spec['methods']), 'Wrong/nonliteral UIKit method roster')
    ...
    require(isinstance(expected, dict) and set(expected) == set(pins['testSwiftInputs']) |
            ...
            and len(pins['testSwiftInputs']) == len(set(pins['testSwiftInputs'])) == 24
            and all(name.endswith('.swift') for name in pins['testSwiftInputs'])
...
def argv(c):
    ...
    return ['/usr/bin/xcodebuild', 'test', '-project', str(c['roots']['app'] / PROJECT), '-scheme', TARGET,
        ...
        *['-only-testing:' + TARGET + '/' + spec['class'] + '/' + method
          for spec in c['uikitSource']['suites'] for method in spec['methods']],
...
def case_results(c, values):
    ...
    for kind, name in (('Test Plan', TARGET), ('Unit test bundle', TARGET)):
        require(len(nodes) == 1 and nodes[0]['nodeType'] == kind and nodes[0]['name'] == name
                and nodes[0]['result'] == 'Passed', 'Wrong/multiple UIKit plan/bundle')
        nodes = nodes[0]['children']
    classes = {spec['class']: spec['methods'] for spec in c['uikitSource']['suites']}
    require(len(nodes) == len(classes) and {node['name'] for node in nodes} == set(classes),
            'Wrong/missing/duplicate UIKit class suites')
    cases = []
    for suite in nodes:
        cls, methods = suite['name'], classes[suite['name']]
        require(suite['nodeType'] == 'Test Suite' and suite['result'] == 'Passed', 'Wrong UIKit class result/type')
        children = suite['children']
        require(len(children) == len(methods) and {node['name'] for node in children} == {name + '()' for name in methods},
        ...
        for node in children:
            ...
            require(node['nodeType'] == 'Test Case' and node['result'] == 'Passed' and not node.get('children')
                    and node['nodeIdentifier'] == cls + '/' + method + '()'
                    and node['nodeIdentifierURL'] == 'test://com.apple.xcode/' + TARGET + '/' + TARGET + '/' + cls + '/' + method,
                    'Wrong/repeated/hidden UIKit execution')
        cases.extend(children)
    return {'counts': counts, 'cases': cases, 'device': device, 'configuration': config}
