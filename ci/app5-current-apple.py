"""UNBOUND App5 one-shot Apple compile controller. Uses accepted Commands only."""
import hashlib
...
BRANCH = 'remediation/app-5-current-apple-validation-01'
REQUEST = 'ci/app5-current-apple.request.json'
FILES = ('ci/app5-current-apple.py', 'ci/app5-current-apple-evidence.py', 'ci/app5-current-apple.init.gradle',
         'ci/app5-original-engine.init.gradle', 'ci/app8-apple.py', 'ci/app5-current-apple.source-pins.json',
         '.github/workflows/app5-current-apple.yml')
...
PLAN = 'bc1613ab9fb0aeebdd3cd80a8302e7109e2034052eb2fc316259416dadfb010b'
...
def admit():
    ...
    fixed = dict(schema='app5-current-apple-v1', status='PRIMARY_BOUND_FOR_REVIEW',
        authorization='APP5_APPLE_COMPILE_ONE_ATTEMPT_AUTHORIZED',
    ...
    expected = {'GITHUB_ACTIONS': 'true', 'GITHUB_REPOSITORY': APP['repository'], 'GITHUB_EVENT_NAME': 'push',
        ...
        'GITHUB_WORKFLOW_REF': APP['repository'] + '/.github/workflows/app5-current-apple.yml@refs/heads/' + BRANCH}
...
def prepare(c):
    ...
    pins = e.read(CONTROL / 'ci/app5-current-apple.source-pins.json')
    require(pins['status'] == 'PRIMARY_BOUND_FOR_REVIEW' and pins['schema'] == 'app5-current-apple-source-pins-v1',
...
def compile_argv(c):
    ...
    return [str(c['roots']['app'] / 'gradlew'), '-p', str(c['roots']['app']), TASK,
        ...
        '-I', str(CONTROL / 'ci/app5-current-apple.init.gradle'),
...
def compile_once(request):
    ...
    require(CONTROL == workspace / 'control' and run == temporary / ('app5-apple-' + os.environ['GITHUB_RUN_ID'] + '-1')
    ...
    e = load('app5_evidence', CONTROL / 'ci/app5-current-apple-evidence.py')
...
if __name__ == '__main__':
    try:
        ...
        print('APP5 APPLE INCOMPLETE: ' + str(error), file=sys.stderr)
        raise SystemExit(1)
