"""Targeted reply+detail AppleNative22 adapter; uses the existing command owner, not a second controller.
...
CLASSES = ('me.manga.kira.data.remote.complaint.IosComplaintMutationReplyTest',
           ...
           'me.manga.kira.data.remote.complaint.IosComplaintHistoryDetailTest',
           'me.manga.kira.data.remote.complaint.IosComplaintHistoryEngineTest',
           'me.manga.kira.data.remote.complaint.IosComplaintHistoryLifecycleTest',
           'me.manga.kira.data.remote.complaint.IosComplaintHistoryReceiveGuardTest')
COUNTS = (3, 4, 2, 8, 1, 2, 2)
SUITE_TASKS = (TASKS[0],) * 7
...
def source_inputs(c):
    ...
    require(set(binding) == {'schema', 'status', 'app', 'suites'}
            and binding['schema'] == 'app-29-apple-native22-tests-v1'
    ...
    require(isinstance(suites, list) and len(suites) == 7, 'Exactly reply9 plus detail13 Native method selections')
    inventories = {}
    ...
    require([len(files) for files in inventories.values()] == [28], 'Complete ordinary Native test input inventory required')
...
def collect(c):
    ...
    proof['passed'] = bool(c['result'].get('nativeTestsSucceeded') and not proof['errors']
                           and proof['xmlCounts'] == dict(tests=22, failures=0, errors=0, skipped=0))
    proof['status'] = 'NATIVE_RESULT_REVIEW_REQUIRED' if proof['passed'] else 'FAIL'
    proof['qualification'] = 'Only the exact Reply3, MutationEngine4, MutationReceiveGuard2, HistoryDetail8, HistoryEngine1, HistoryLifecycle2 and HistoryReceiveGuard2 definitions: ordinary Native compile/link and Foundation guard behavior. No live Darwin-network execution, native allocation/memory or cancellation-drain qualification, Compose/framework/Swift host/UI, physical-device or shipping proof.'
