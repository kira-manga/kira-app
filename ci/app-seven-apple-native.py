...
def shutdown_simulator(c, udid):
    commands = c['commands']
    first = len(commands.tasks)
    try:
        simctl(c, ['shutdown', udid], 'shutdown-owned-simulator', cleaning=True)
    except Exception as error:
        # A late shutdown stays failed. Only settle its existing owner, then allow
        # the caller's same-UUID/state/root checks to authorize deletion; no retry.
        c['result']['errors'].append('owned simulator shutdown: ' + str(error)[:500])
        require(commands.drain(), 'Failed shutdown owner remains unsettled; retain device')
        receipts = [task['receipt'] for task in commands.tasks[first:]
            if task['receipt']['label'] == 'shutdown-owned-simulator'
            and task['receipt']['argv'] == ['/usr/bin/xcrun', 'simctl', 'shutdown', udid]]
        require(len(receipts) == 1 and receipts[0]['leaderReaped'] and receipts[0]['groupQuiet']
                and receipts[0]['actualExit'] == 0 and not receipts[0]['forced'] and not receipts[0]['ownershipLost'],
                'Failed shutdown has no clean owned exit; retain device')
...
def run_canary(c, binary):
    ...
    # The actual bounded log query below verifies supported syntax and readable evidence.
    # A separate help invocation proves neither and must not block the real stimulus.
