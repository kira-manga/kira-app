"""Targeted ReaderShare5 + Room cancellation2 adapter; uses the existing command owner, not a second controller.
...
TASKS = (':composeApp:iosSimulatorArm64Test', ':data:iosSimulatorArm64Test')
CLASSES = ('me.manga.kira.reader.ReaderShareCoordinatorTest', 'me.manga.kira.data.repository.IosRoomCancellationIntegrityTest')
COUNTS = (5, 2)
SUITE_TASKS = TASKS
...
def source_inputs(c):
    ...
    require(set(binding) == {'schema', 'status', 'app', 'suites'}
            and binding['schema'] == 'app-125456-apple-native-tests-v1'
    ...
    require(isinstance(suites, list) and len(suites) == 2, 'Exactly ReaderShare and Room cancellation Native suites')
    inventories = {}
    ...
    require([len(files) for files in inventories.values()] == [51, 34], 'Complete ordinary Native test input inventories required')
...
def task_proof(c):
    ...
    require({task.replace('iosSimulatorArm64Test', 'compileKotlinIosSimulatorArm64') for task in TASKS}
            <= set(main_paths), 'Missing ordinary ComposeApp main producer')
    mains = {}
...
def collect(c):
    ...
    proof['passed'] = bool(c['result'].get('nativeTestsSucceeded') and not proof['errors']
                           and proof['xmlCounts'] == dict(tests=7, failures=0, errors=0, skipped=0))
    proof['status'] = 'NATIVE_RESULT_REVIEW_REQUIRED' if proof['passed'] else 'FAIL'
    proof['qualification'] = 'Only5 new ReaderShareCoordinator admission/lifecycle probes and2 real-Room cancellation/rollback/joined retry/reopen methods on Native; direct framework evidence is separate. No original30, bitmap encoder, screenshot integration, Swift host, UIKit runtime or physical-device proof.'
