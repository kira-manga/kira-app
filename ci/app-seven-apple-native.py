"""Targeted mobile-edit AppleNative13 adapter; uses the existing command owner, not a second controller.
...
CLASSES = ('me.manga.kira.data.remote.complaint.IosComplaintMutationEditTest',
           'me.manga.kira.data.remote.complaint.IosComplaintMutationEditReceiveGuardTest',
...
def source_inputs(c):
    ...
    require(set(binding) == {'schema', 'status', 'app', 'suites'}
            and binding['schema'] == 'app-29-apple-native13-tests-v1'
    ...
    require(isinstance(suites, list) and len(suites) == 4, 'Exactly edit9 plus four affected Native method selections')
    inventories = {}
    ...
    require([len(files) for files in inventories.values()] == [32], 'Complete ordinary Native test input inventory required')
...
def collect(c):
    ...
    proof['passed'] = bool(c['result'].get('nativeTestsSucceeded') and not proof['errors']
                           and proof['xmlCounts'] == dict(tests=13, failures=0, errors=0, skipped=0))
    proof['status'] = 'NATIVE_RESULT_REVIEW_REQUIRED' if proof['passed'] else 'FAIL'
    proof['qualification'] = 'Only the exact Edit4, EditReceiveGuard5, MutationEngine3 and MutationReceiveGuard1 definitions: ordinary Native compile/link and Foundation guard behavior. No live Darwin-network execution, native allocation/memory or cancellation-drain qualification, Compose/framework/Swift host/UI, physical-device or shipping proof.'
