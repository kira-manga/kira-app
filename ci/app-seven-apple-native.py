"""Targeted deletion AppleNative8 adapter; uses the existing command owner, not a second controller.
...
CLASSES = ('me.manga.kira.data.remote.complaint.IosComplaintDeletionEngineTest',
           'me.manga.kira.data.remote.complaint.IosComplaintDeletionReceiveGuardTest')
COUNTS = (4, 4)
SUITE_TASKS = (TASKS[0], TASKS[0])
...
def source_inputs(c):
    ...
    require(set(binding) == {'schema', 'status', 'app', 'suites'}
            and binding['schema'] == 'app-29-apple-native8-tests-v1'
    ...
    require(isinstance(suites, list) and len(suites) == 2, 'Exactly deletion engine4 and receive-guard4 Native suites')
    inventories = {}
    ...
    require([len(files) for files in inventories.values()] == [24], 'Complete ordinary Native test input inventory required')
...
def task_arguments(c):
    ...
    for task in TASKS:
        ...
        for spec in c['nativeSource']['suites']:
            if spec['task'] == task:
                result.extend(('--tests', spec['class'].rsplit('.', 1)[-1] + '.*'))
...
def task_proof(c):
    ...
    for task in TASKS:
        ...
        selectors = sorted(spec['class'].rsplit('.', 1)[-1] + '.*' for spec in c['nativeSource']['suites'] if spec['task'] == task)
...
def collect(c):
    ...
    proof['passed'] = bool(c['result'].get('nativeTestsSucceeded') and not proof['errors']
                           and proof['xmlCounts'] == dict(tests=8, failures=0, errors=0, skipped=0))
    proof['status'] = 'NATIVE_RESULT_REVIEW_REQUIRED' if proof['passed'] else 'FAIL'
    proof['qualification'] = 'Only the exact deletion Engine4 and ReceiveGuard4 definitions: ordinary Native compile/link and Foundation guard behavior. No live Darwin-network execution, native allocation/memory or cancellation-drain qualification, Compose/framework/Swift host/UI, physical-device or shipping proof.'
