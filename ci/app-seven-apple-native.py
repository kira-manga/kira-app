"""Targeted 15-case Native adapter; uses the existing command owner, not a second controller.
...
CLASSES = ('me.manga.kira.platform.cbz.IosCbzCompletenessTest',
           ...
           'me.manga.kira.platform.media.IosPngIntegrityTest',
...
COUNTS = (1, 1, 4, 2, 4, 1, 2)
SUITE_TASKS = (TASKS[0],) * 5 + TASKS[1:]
...
def source_inputs(c):
    ...
    require(isinstance(suites, list) and len(suites) == 7, 'Exactly seven targeted native suites')
    inventories = {}
...
def collect(c):
    ...
    proof['passed'] = bool(c['result'].get('nativeTestsSucceeded') and not proof['errors']
                           and proof['xmlCounts'] == dict(tests=15, failures=0, errors=0, skipped=0))
    proof['status'] = 'NATIVE_RESULT_REVIEW_REQUIRED' if proof['passed'] else 'FAIL'
    proof['qualification'] = 'Targeted Native platform12, Room finalization1 and manifest-filesystem2 only; not a full-suite rerun, UIKit runtime or physical-device proof.'
