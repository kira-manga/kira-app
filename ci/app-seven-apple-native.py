"""Lean 57-case Native adapter; uses the existing command owner, not a second controller.
...
TASKS = (':platform:iosSimulatorArm64Test', ':data:iosSimulatorArm64Test', ':data:download:iosSimulatorArm64Test')
CLASSES = ('me.manga.kira.platform.image.IosAvifDecoderTest',
           'me.manga.kira.platform.image.IosAvifDecoderFailureTest',
           'me.manga.kira.platform.cbz.IosCbzNativeOwnershipTest',
           'me.manga.kira.platform.cbz.IosCbzMemoryAdmissionTest',
           'me.manga.kira.platform.cbz.IosCbzCompletenessTest',
           'me.manga.kira.platform.cbz.IosCbzReaderValidationTest',
           'me.manga.kira.platform.media.IosPageMediaInspectorTest',
           'me.manga.kira.platform.download.IosBackgroundTransportTest',
           'me.manga.kira.platform.download.IosBackgroundCompletionTest',
           'me.manga.kira.data.repository.IosCbzFinalizationTest',
           'me.manga.kira.presentation.features.download.domain.clean.DownloadManifestStoreTest')
COUNTS = (7, 3, 6, 8, 8, 2, 5, 11, 4, 1, 2)
SUITE_TASKS = (TASKS[0],) * 9 + TASKS[1:]
...
def source_inputs(c):
    ...
    require(isinstance(suites, list) and len(suites) == 11, 'Exactly eleven native suites')
    inventories = {}
...
def collect(c):
    ...
    for spec in c['nativeSource']['suites']:
        try:
            ...
            expected = sorted(folder / ('TEST-iosSimulatorArm64Test.' + row['class'] + '.xml')
                              for row in c['nativeSource']['suites'] if row['task'] == spec['task'])
            require(paths == expected, 'Missing/extra ordinary XML suites: ' + module)
            raw = c['h'].bytes_of(folder / filename, 1048576)
    ...
    proof['passed'] = bool(c['result'].get('nativeTestsSucceeded') and not proof['errors']
                           and proof['xmlCounts'] == dict(tests=57, failures=0, errors=0, skipped=0))
    proof['status'] = 'NATIVE_RESULT_REVIEW_REQUIRED' if proof['passed'] else 'FAIL'
    proof['qualification'] = 'Native platform54, Room finalization1 and manifest-filesystem2 only; no UIKit runtime or physical-device proof.'
