"""Targeted mobile-owner-delete AppleNative12; uses the existing command owner, not a second controller.
...
TASKS = (':data:remote:iosSimulatorArm64Test',)
CLASSES = ('me.manga.kira.data.remote.complaint.IosComplaintMutationOwnerDeleteTest',
           'me.manga.kira.data.remote.complaint.IosComplaintMutationOwnerDeleteReceiveGuardTest',
           'me.manga.kira.data.remote.complaint.IosComplaintMutationEngineTest',
           'me.manga.kira.data.remote.complaint.IosComplaintMutationReceiveGuardTest')
COUNTS = (4, 5, 2, 1)
SUITE_TASKS = (TASKS[0],) * 4
...
def source_inputs(c):
    ...
    require(set(binding) == {'schema', 'status', 'app', 'suites'}
            and binding['schema'] == 'app-29-apple-native12-tests-v1'
    ...
    require(isinstance(suites, list) and len(suites) == 4, 'Exactly owner-delete9 plus three affected Native method selections')
    inventories = {}
    ...
    require([len(files) for files in inventories.values()] == [36], 'Complete ordinary Native test input inventory required')
...
def create_simulator(c):
    state = c['simulator']
    runtimes = json.loads(simctl(c, ['list', 'runtimes', '--json'], 'runtimes', seconds=60))['runtimes']
...
def dispose_simulator(c):
    ...
    if bound is not None:
        ...
        if state['bootIntended']:
            simctl(c, ['shutdown', udid], 'shutdown-owned-simulator', seconds=60, cleaning=True)
...
def swiftpm_proof(c, report):
    e, root = c['e'], c['roots']['app']
    metadata = {}
    for module in e.SWIFTPM_MODULES:
        task = ':' + module.replace('/', ':') + ':serializeSwiftPMDependenciesMetadataForLockFiles'
        row = report['observed'][task]
        path = root / module / e.SWIFTPM_METADATA
        require(e.completed(report['states'][task]) and row['dependenciesEmpty'] is True
                and row['directPackages'] == [] and all(item['packages'] == [] for item in row['transitiveMetadata'])
                and row['outputPath'] == str(path) and e.fingerprint(path, c['end'], 65536) == row['output'],
                'Missing/nonempty/changed actual SwiftPM lock metadata: ' + task)
        metadata[task] = row['output']
    for task in e.SWIFTPM_SKIPPED:
        row, state = report['observed'][task], report['states'][task]
        require(row['actionAuthorized'] is False and not row.get('unexpectedAction') and state['skipped'] is True
                and not any(state[key] for key in ('didWork', 'upToDate', 'noSource', 'failure')),
                'SwiftPM generate/fetch/sync must naturally skip: ' + task)
        if 'directPackages' in row:
            require(row['dependenciesEmpty'] is True and row['directPackages'] == []
                    and all(item['packages'] == [] for item in row['transitiveMetadata']), 'Nonempty SwiftPM generator inputs')
    local = ':data:remote:computeLocalPackageDependencyInputFiles'
    defs = ':data:remote:convertSyntheticImportProjectIntoDefFileIphonesimulator'
    require(all(e.completed(report['states'][task]) and report['observed'][task]['dependenciesEmpty'] is True
                for task in (local, defs)) and report['observed'][local]['localPackageCount'] == 0
            and report['observed'][local]['output']['bytes'] == 0
            and report['observed'][defs]['hasSwiftPMDependencies'] is False,
            'Only ordinary empty local-input/no-dependency def generation is authorized')
    require(e.fingerprint(root / 'data/remote/build/kotlin/swiftImportFilesToTrackFromLocalPackages', c['end'], 65536)
            == report['observed'][local]['output'], 'Changed ordinary empty local-input file')
    require(not (root / '.swiftpm-locks').exists() and not (root / '.swiftpm-locks').is_symlink(),
            'Unexpected SwiftPM persisted/generated material')
    return {'metadataOutputs': metadata, 'naturallySkipped': list(e.SWIFTPM_SKIPPED),
            'localPackages': 0, 'swiftPackageResolveAuthorized': False, 'swiftPMXcodeBuildAuthorized': False}
...
def task_proof(c):
    ...
    require(set(main_paths) == {':core:compileKotlinIosSimulatorArm64', ':data:remote:compileKotlinIosSimulatorArm64'},
            'Exactly the ordinary core and data:remote main producers')
    mains = {}
    for task in main_paths:
        ...
        mains[task] = compiled
    result = {'ordinaryMains': mains, 'swiftPMPrerequisites': swiftpm_proof(c, report)}
    for task in TASKS:
        module = task.rsplit(':', 1)[0][1:].replace(':', '/')
        compile_task = task.replace('iosSimulatorArm64Test', 'compileTestKotlinIosSimulatorArm64')
        link_task = task.replace('iosSimulatorArm64Test', 'linkDebugTestIosSimulatorArm64')
        selectors = sorted(spec['class'] + '.' + method for spec in c['nativeSource']['suites']
                           if spec['task'] == task for method in spec['methods'])
...
def xml_identity(suite, spec):
    cases, cls = suite.findall('testcase'), 'iosSimulatorArm64Test.' + spec['class']
    return (suite.tag == 'testsuite' and suite.get('name') == cls
...
def xml_diagnostics(c):
    # Supplier basenames may be shortened. Exact XML identities choose the four
    # canonical slots; raw bytes and original names/digests remain evidence.
    diagnostic = {'roots': [], 'errors': [], 'maximumNamesPerRoot': 32, 'unexpectedSlotsPerRoot': 4,
                  'maximumXmlReadsPerRoot': 8, 'lookup': 'EXACT_XML_SUITE_CLASS_METHODS'}
    for task in TASKS:
        module = task.rsplit(':', 1)[0][1:].replace(':', '/')
        specs = [spec for spec in c['nativeSource']['suites'] if spec['task'] == task]
        expected = sorted('TEST-iosSimulatorArm64Test.' + spec['class'] + '.xml'
                          for spec in specs)
        row, matched = {'task': task, 'expectedSlots': expected, 'observed': []}, set()
        ...
        try:
            ...
            extra = 0
            for index, path in enumerate(paths[:32]):
                ...
                try:
                    ...
                    if index >= diagnostic['maximumXmlReadsPerRoot']:
                        item['notRetained'] = 'XML read slot limit'
                        continue
                    raw, suite = c['h'].bytes_of(path, 1048576), None
                    try:
                        require(b'<!DOCTYPE' not in raw and b'<!ENTITY' not in raw, 'Unexpected XML declarations')
                        suite = ET.fromstring(raw)
                    except Exception as error:
                        item['error'] = str(error)[:500]
                    matches = [spec for spec in specs if suite is not None and xml_identity(suite, spec)]
                    slot = 'TEST-iosSimulatorArm64Test.' + matches[0]['class'] + '.xml' if len(matches) == 1 else None
                    if slot is not None and slot not in matched:
                        matched.add(slot)
                        name = 'native-xml/' + module + '/' + slot
                    else:
                        extra += 1
                        if extra > 4:
                    ...
                    item['error'] = str(error)[:500]
            require(len(paths) == len(expected) and matched == set(expected),
...
def collect(c):
    ...
    diagnostic = xml_diagnostics(c)
    proof['errors'].extend(diagnostic['errors'])
    for spec in c['nativeSource']['suites']:
        try:
            module = spec['task'].rsplit(':', 1)[0][1:].replace(':', '/')
            filename = 'TEST-iosSimulatorArm64Test.' + spec['class'] + '.xml'
            name = 'native-xml/' + module + '/' + filename
            origins = [item for root in diagnostic['roots'] if root['task'] == spec['task']
                       for item in root['observed'] if item.get('retained') == name]
            require(len(origins) == 1, 'Missing/duplicate retained XML identity mapping')
            origin = origins[0]
            raw = c['h'].bytes_of(c['reports'] / name, 1048576)
            require(len(raw) == origin['bytes'] and hashlib.sha256(raw).hexdigest() == origin['sha256'],
                    'Retained XML bytes differ from the original supplier file')
            require(b'<!DOCTYPE' not in raw and b'<!ENTITY' not in raw, 'Unexpected XML declarations')
            suite = ET.fromstring(raw)
            ...
            row = {'task': spec['task'], 'class': suite.get('name'), 'counts': counts, 'xml': name,
                   'originalFilename': origin['filename'], 'sha256': hashlib.sha256(raw).hexdigest(),
            ...
            require(xml_identity(suite, spec) and all(counts[key] == 0 for key in ('failures', 'errors', 'skipped'))
    ...
    proof['passed'] = bool(c['result'].get('nativeTestsSucceeded') and not proof['errors']
                           and proof['xmlCounts'] == dict(tests=12, failures=0, errors=0, skipped=0))
    proof['status'] = 'NATIVE_RESULT_REVIEW_REQUIRED' if proof['passed'] else 'FAIL'
    proof['qualification'] = 'Only the exact OwnerDelete4, OwnerDeleteReceiveGuard5, MutationEngine2 and MutationReceiveGuard1 definitions: ordinary Native compile/link and Foundation guard behavior. No live Darwin-network execution, native allocation/memory or cancellation-drain qualification, Compose/framework/Swift host/UI, physical-device or shipping proof.'
