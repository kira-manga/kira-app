"""Targeted 30-case Native adapter; uses the existing command owner, not a second controller.
...
TASKS = (':platform:iosSimulatorArm64Test', ':data:download:iosSimulatorArm64Test', ':composeApp:iosSimulatorArm64Test')
CLASSES = ('me.manga.kira.platform.media.IosPngIntegrityTest',
           'me.manga.kira.platform.notification.IosDownloadNotifierContentTest',
           'me.manga.kira.presentation.features.download.domain.clean.DownloadManifestStoreTest',
           'me.manga.kira.di.IosSourceActivationBridgeTest',
           'me.manga.kira.reader.ReaderNativeLifecycleTest',
           'me.manga.kira.reader.ReaderNativeEffectDispatchTest')
COUNTS = (4, 6, 2, 6, 10, 2)
SUITE_TASKS = (TASKS[0], TASKS[0], TASKS[1], TASKS[2], TASKS[2], TASKS[2])
...
def source_inputs(c):
    ...
    require(isinstance(suites, list) and len(suites) == 6, 'Exactly six targeted native suites')
    inventories = {}
    ...
    require([len(files) for files in inventories.values()] == [30, 13, 50], 'Complete ordinary platform30/download13/composeApp50 test input files required')
...
def task_proof(c):
    ...
    require({task.replace('iosSimulatorArm64Test', 'compileKotlinIosSimulatorArm64') for task in TASKS}
            <= set(main_paths), 'Missing ordinary platform/download/ComposeApp main producers')
    mains = {}
...
def collect(c):
    ...
    proof['passed'] = bool(c['result'].get('nativeTestsSucceeded') and not proof['errors']
                           and proof['xmlCounts'] == dict(tests=30, failures=0, errors=0, skipped=0))
    proof['status'] = 'NATIVE_RESULT_REVIEW_REQUIRED' if proof['passed'] else 'FAIL'
    proof['qualification'] = 'Targeted Native PNG4, notifier-content6, manifest-filesystem2, activation-bridge6, reader-lifecycle10 and reader-effects2 only; host build evidence is separate, not UIKit runtime or physical-device proof.'
