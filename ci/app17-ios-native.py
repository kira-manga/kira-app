"""Public App17 Native04 link-first Native3 preparation; admission remains UNBOUND."""
import hashlib
...
BRANCH = 'remediation/app-17-public-native-validation-04'
...
BUILD_TASKS = tuple(SOURCE_ROOTS) + (CINTEROP, LINK_TASK)
REQUIRED_TASKS = BUILD_TASKS + (TASK,)
LINK_RECEIPTS = ('source.json', 'compilation-inputs.json', 'native-closure.json', 'link-inputs.json', 'linked-binary.json')
CONTROL_FILES = {
...
PUBLIC_JSON = ('request.json', 'result.json', 'carrier.json', 'source.json', 'source-after.json',
    'tools.json', 'simulator.json', 'simulator-cleanup.json', 'native-proof.json',
...
PUBLIC_LOGS = {'native3': ('native-link', 'native3'),
    'gradle-stop-immediate': ('gradle-stop-link', 'gradle-stop-immediate'),
    'gradle-stop-final': ('gradle-stop-final',), 'owned-bootstatus': ('owned-bootstatus',)}
...
def validate_request(request, context):
    require(BRANCH != 'UNBOUND', 'Primary branch binding is UNBOUND')
    ...
    expected = {'GITHUB_ACTIONS': 'true', 'GITHUB_REPOSITORY': 'kira-manga/kira-app',
                ...
                'GITHUB_WORKFLOW_REF': 'kira-manga/kira-app/.github/workflows/app17-ios-native.yml@refs/heads/' + BRANCH}
...
def gradle_argv(source, run, phase, udid=None):
    require(phase in ('link', 'test') and (udid is None) == (phase == 'link'), 'Wrong Native phase/device binding')
    filters = [value for selector in SELECTORS for value in ('--tests', selector)]
    root = [LINK_TASK] if phase == 'link' else [TASK, *filters, '--device', udid]
    return [str(source / 'gradlew'), '-p', str(source), *root,
...
def create_simulator(owner, commands, runner_home, state, work_end):
    runtimes = json.loads(simctl(commands, runner_home, ['list', 'runtimes', '--json'], 'runtimes', seconds=60, end=work_end))['runtimes']
...
def phase_proof(reports, request, phase, task):
    proof = read_json(reports / 'task-proof.json')[phase]
    require(proof['schema'] == 'app17-ios-native-task-v1'
            and proof['phase'] == phase and proof['run'] == reports.parent.name
            and proof['requestedTask'] == task and proof['sourceSha'] == request['sourceSha']
            and proof['gradleVersion'] == '9.6.1', 'Missing real task proof')
    return proof


def completed_action(row):
    return (all(row[key] is True for key in ('executed', 'didWork', 'actionsReachedEnd')) and row['failure'] is None
            and all(row[key] is False for key in ('skipped', 'upToDate', 'noSource')))


def link_proof(source, reports, request):
    proof = phase_proof(reports, request, 'link', LINK_TASK)
    require(TASK not in proof['graph'] and all(completed_action(proof['states'][task]) for task in BUILD_TASKS),
            'Phase1 must complete every real compile/cinterop/link action, never tests')
    ...
    for row in inputs['libraries']:
        actual = fingerprint(Path(row['file']))
        require(all(row[key] == value for key, value in actual.items()), 'Changed actual Native link input')
        outputs[row['file']] = actual
    ...
    outputs[str(binary)] = fingerprint(binary)
    return proof, outputs, linked


def native_binary_proof(binary, commands, work_end):
    arch = commands.call(['/usr/bin/xcrun', 'lipo', '-archs', str(binary)], 'native-arch', seconds=15, end=work_end).strip()
    build = commands.call(['/usr/bin/xcrun', 'vtool', '-show-build', str(binary)], 'native-platform', seconds=15, end=work_end)
    ...
    return {'architecture': arch, 'platform': 'IOSSIMULATOR', 'sdk': '26.4', 'observedMinimumOS': minimum[0]}


def bind_link_phase(owner, source, reports, request, result, commands, work_end):
    proof, outputs, linked = link_proof(source, reports, request)
    original = read_json(reports / 'source.json')
    require(source_inventory(source) == original['compilations'] and
            {name: digest(source / name) for name in original['inputs']} == source_inputs(), 'Source changed during phase1')
    verify_controls(request)
    binary = native_binary_proof(Path(linked['file']), commands, work_end)
    require(result['afterLinkStop']['absent'] and commands.normal() and not result['errors']
            and time.monotonic() < work_end, 'No phase handoff without normal owned phase1 completion')
    binding = {'schema': 'app17-native-link-binding-v1', 'sourceSha': request['sourceSha'], 'sourceTree': TREE,
        'carrierSha': os.environ['GITHUB_SHA'], 'sourceRoot': str(source), 'run': reports.parent.name,
        'native3AtPhase1': 'NOT_RUN', 'afterLinkStop': result['afterLinkStop'], 'normalPhase1Completion': True,
        'taskProof': proof, 'taskGraph': read_json(reports / 'task-graph.json')['link'],
        'receipts': {name: digest(reports / name) for name in LINK_RECEIPTS}, 'outputs': outputs,
        'linkedBinary': linked, 'nativeBinary': binary, 'nativeMarkers': native_markers(reports.parent)}
    owner.save(reports / 'native-proof.json', binding, limit=1048576)
    return digest(reports / 'native-proof.json')


def verify_link_binding(source, reports, request, expected_sha):
    require(digest(reports / 'native-proof.json') == expected_sha, 'Phase1 binding bytes changed')
    binding = read_json(reports / 'native-proof.json')
    require(binding['schema'] == 'app17-native-link-binding-v1' and binding['sourceSha'] == request['sourceSha']
            and binding['sourceTree'] == TREE and binding['carrierSha'] == os.environ['GITHUB_SHA']
            and binding['sourceRoot'] == str(source) and binding['run'] == reports.parent.name
            and binding['normalPhase1Completion'] is True and binding['afterLinkStop']['absent'] is True,
            'Wrong same-run phase1 binding')
    require(set(binding['receipts']) == set(LINK_RECEIPTS) and all(digest(reports / name) == sha
            for name, sha in binding['receipts'].items()), 'Phase1 compiler/link/source receipts changed')
    require(binding['taskProof'] == phase_proof(reports, request, 'link', LINK_TASK)
            and binding['taskGraph'] == read_json(reports / 'task-graph.json')['link'], 'Phase1 task history changed')
    require(all(fingerprint(Path(path)) == row for path, row in binding['outputs'].items()), 'Phase1 outputs changed')
    original = read_json(reports / 'source.json')
    require(source_inventory(source) == original['compilations'] and
            {name: digest(source / name) for name in original['inputs']} == source_inputs(), 'Source changed between phases')
    return binding


def output_proof(source, reports, request, state, commands, binding_sha):
    binding = verify_link_binding(source, reports, request, binding_sha)
    proof = phase_proof(reports, request, 'test', TASK)
    require(completed_action(proof['states'][TASK]), 'Unproven actual Native3 test action')
    for task in BUILD_TASKS:
        row = proof['states'][task]
        require(row['reusedFromPhase'] == 'link' and row['linkBindingSha256'] == binding_sha
                and all(row[key] is True for key in ('executed', 'upToDate', 'skipped'))
                and all(row[key] is False for key in ('didWork', 'noSource', 'actionsReachedEnd'))
                and row['failure'] is None, 'Unproven same-run prerequisite reuse: ' + task)
    linked = binding['linkedBinary']
    invocation = read_json(reports / 'test-invocation.json')
    require(invocation['task'] == TASK and invocation['sourceSha'] == request['sourceSha']
            and invocation['phase'] == 'test' and invocation['linkBindingSha256'] == binding_sha
            ...
            and invocation['executable'] == linked['file'] and invocation['sha256'] == linked['sha256'], 'Wrong actual Native invocation')
    ...
    return {'tests': [case.attrib for case in cases], 'xmlSha256': digest(xmls[0]), 'linkedSha256': linked['sha256'],
            'linkBindingSha256': binding_sha, **binding['nativeBinary'],
...
def public_log(commands, reports, result, name, labels):
    tasks = []
    for label in labels:
    ...
    if not tasks:
        return None
    target, parts, segments = reports / (name + '.log'), [], []
    require(not target.exists() and not target.is_symlink(), 'Preexisting public log alias')
    for task in tasks:
        label = task['receipt']['label']
        source = task['log']
        ...
        require(source.is_file() and not source.is_symlink() and source.resolve() == source
                and source.stat().st_size <= 1048576,
                'Unsafe/oversized public log alias')
        with source.open('rb') as stream:
            data = stream.read(1048577)
        require(len(data) <= 1048576, 'Public log source grew after absence')
        require(hashlib.sha256(data).hexdigest() == digest(source), 'Owned public log source changed')
        prefix = data[:1048576 // len(tasks)]
        if len(prefix) != len(data):
            result['errors'].append('Combined public log cap exceeded: ' + source.name)
        segments.append({'ownedCommandOutput': source.name, 'offset': sum(map(len, parts)), 'bytes': len(prefix),
            'sha256': hashlib.sha256(prefix).hexdigest(), 'boundedSourceSha256': digest(source),
            'originalBytes': result['logBytesBeforeRetentionCap'][source.name]})
        parts.append(prefix)
    with target.open('xb') as stream:
        stream.write(b''.join(parts))
    require(len(tasks) != 1 or digest(target) == segments[0]['boundedSourceSha256'], 'Exact single-log alias changed')
    return {'sha256': digest(target), 'bytes': target.stat().st_size, 'segments': segments}


def public_retention(commands, reports, result):
    # Exact owned bootstatus bytes; only the two phase/stop pairs share bounded public files.
    require(result.get('afterFinalStop', {}).get('absent') is True, 'No public retention before final owned absence')
    aliases = {}
    for name, labels in PUBLIC_LOGS.items():
        alias = public_log(commands, reports, result, name, labels)
        if alias is not None:
            aliases[name + '.log'] = alias
    present = []
...
def run_recipe(owner, source, run, request, system, runner_home):
    ...
    result = {'schema': 'app17-ios-native-public-result-v1', 'passed': False, 'sourceSha': request['sourceSha'],
              ...
              'linkAttempted': False, 'nativeAttempted': False, 'native3Status': 'NOT_RUN',
    ...
    def stop(label, by=None):
        try:
            commands.retire_observer()
            commands.call([str(source / 'gradlew'), '--stop'], label, seconds=40, end=by, cleaning=True)
        except Exception as error:
    ...
    try:
        ...
        env['APP17_PHASE'] = 'link'
        started = result['linkAttempted'] = True
        try:
            commands.call(gradle_argv(source, run, 'link'), 'native-link', seconds=1200, end=work_end)
            result['linkSucceeded'] = True
        finally:
            stop('gradle-stop-link', work_end)
            ...
            result['afterLinkStop'] = absence(commands, run, source, state)
            require(result['afterLinkStop']['absent'], 'Owned phase1 workers remain; no simulator/phase2')
        binding_sha = bind_link_phase(owner, source, reports, request, result, commands, work_end)
        result.update(linkProofPreserved=True, linkBindingSha256=binding_sha)
        owner.save(reports / 'result.json', result, limit=1048576)
        create_simulator(owner, commands, runner_home, state, work_end)
        verify_link_binding(source, reports, request, binding_sha)
        verify_controls(request)
        env['APP17_DEVICE'] = state['udid']
        env['APP17_PHASE'] = 'test'
        env['APP17_LINK_BINDING_SHA256'] = binding_sha
        result['nativeAttempted'] = True
        commands.call(gradle_argv(source, run, 'test', state['udid']), 'native3', seconds=1200, end=work_end)
            ...
            if source_ready and started:
                try:
                    if result.get('nativeSucceeded'):
                        result['native3Proof'] = output_proof(source, reports, request, state, commands, binding_sha)
        ...
        result['native3Status'] = ('RESULT_REVIEW_REQUIRED' if result.get('outputProofPreserved') else
                                   ('UNPROVEN' if result['nativeAttempted'] else 'NOT_RUN'))
        ...
        remaining = 4194304
        for path in sorted(reports.glob('*.log'), key=lambda p: (not p.name.endswith('-owned-bootstatus.log'), p.name)):
        ...
        result['passed'] = bool(result.get('linkSucceeded') and result.get('linkProofPreserved')
                                and result.get('afterLinkStop', {}).get('absent')
                                and result.get('nativeSucceeded') and result.get('outputProofPreserved')
