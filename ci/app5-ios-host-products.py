...
def framework(c, app, s):
    ...
    e.save(c['reports'] / 'framework-metadata.json', {
        ...
        'metadataObservations': [
            ...
            metadata_observation('exported-header', exported / 'Headers/ComposeApp.h', 4194304),
    ...
    header, module = h.bytes_of(original / 'Headers/ComposeApp.h', 4194304), h.bytes_of(original / 'Modules/module.modulemap', 65536)
    require(header == h.bytes_of(exported / 'Headers/ComposeApp.h', 4194304) and
...
def swift_inputs(c, raw, fw):
    ...
    require(len(expected) == 21 and len(names) == len(set(names)) and expected <= set(names)
...
def built_plist(c, s):
    ...
    firebase = c['h'].bytes_of(bundle / 'GoogleService-Info.plist', 65536)
    # Xcode copyPlist converts XML to binary; compare the complete typed plist, not its encoding.
    require(plistlib.dumps(plistlib.loads(firebase), sort_keys=True) ==
            plistlib.dumps(plistlib.loads(example), sort_keys=True), 'Built Firebase example mismatch')
    return {'path': str(path), 'sha256': hashlib.sha256(data).hexdigest(), 'identity': {
        ...
        'firebaseBuiltSha256': hashlib.sha256(firebase).hexdigest()}
