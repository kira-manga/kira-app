...
def producer_joins(c, proof, app):
    ...
    expected.append(app['observed'][':platform:cinteropLibwebpIosSimulatorArm64']['output'])
    require({r['file'] for r in expected} <= set(row['inputs']), 'A real main/Engine/libwebp producer is missing from link inputs')
    neutral = row['neutral']
    ...
    return {'mainAndInteropInputs': expected, 'originalEngine': neutral, 'embedTaskState': embed,
...
def framework(c, app, s):
    ...
    require(c['commands'].call(['/usr/bin/xcrun', 'lipo', '-archs', str(exported_binary)],
            'framework-arch', end=c['end'], cleaning=True).strip() == 'arm64', 'Wrong static framework architecture')
...
def swift_command(raw, filelist, framework_dir):
    rows = []
    for line in raw.splitlines():
        if str(filelist) not in line or 'swiftc' not in line:
            continue
        tokens = shlex.split(line)
        if any(Path(value).name == 'swiftc' for value in tokens) and '@' + str(filelist) in tokens:
            rows.append(tokens)
    require(len(rows) == 1, 'No unique actual shipping Swift driver invocation')
    ...
    return {'argv': rows[0], 'frameworkSearchPaths': frameworks, 'headerSearchPaths': search_paths(rows[0], '-I')}
...
def swift_inputs(c, raw, fw):
    ...
    require(len(expected) == 14 and len(names) == len(set(names)) and expected <= set(names)
    ...
    return {'fileList': str(lists[0]), 'sha256': c['e'].sha(lists[0]), 'shippingInputs': sorted(expected),
            'compiler': swift_command(raw, lists[0], fw['frameworkDirectory']),
...
def package_checkouts(c):
    ...
    for child in sorted(root.iterdir()):
        ...
        revision = c['commands'].call(['/usr/bin/git', '-C', str(child), 'rev-parse', 'HEAD'],
            'package-head-' + str(len(rows)), seconds=10, end=c['end'], cleaning=True).strip()
