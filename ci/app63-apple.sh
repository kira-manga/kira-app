...
compile_code=NOT_RUN; compile_stop_code=NOT_RUN
cleanup() {
  ...
  mkdir -p reports/xml/data/download reports/xml/data
  find app/data/download/build/test-results/iosSimulatorArm64Test -name '*.xml' -exec cp {} reports/xml/data/download/ \; 2>/dev/null
...
# Exact five changed/new store methods only; do not replay its ten unchanged methods.
# --continue retains this independent module if data test compilation fails.
set +e
(cd "$APP" && ./gradlew :data:download:iosSimulatorArm64Test \
  --tests me.manga.kira.presentation.features.download.domain.clean.DownloadManifestStoreTest.read_corruptManifest_refusesInsteadOfResettingTheDurableRoster \
  --tests me.manga.kira.presentation.features.download.domain.clean.DownloadManifestStoreTest.unreadableManifestDoesNotBecomeMissingOrReportAnUncommittedAttempt \
  --tests me.manga.kira.presentation.features.download.domain.clean.DownloadManifestStoreTest.orphanStageCleanupIsExactChapterScopedAndNeverRecursive \
  --tests me.manga.kira.presentation.features.download.domain.clean.DownloadManifestStoreTest.orphanCleanupRefusesASymlinkedChapterInsteadOfDeletingItsTarget \
  --tests me.manga.kira.presentation.features.download.domain.clean.DownloadManifestStoreTest.incrementAttempt_missingManifest_refusesAnUncommittedZeroBudget \
  ...
  --tests me.manga.kira.data.repository.IosManifestPersistenceTest \
  --tests me.manga.kira.data.repository.IosManifestRecoverySafetyTest --continue \
...
# One ordinary Kotlin iOS composition compile, independent of test outcome. No generated
# Xcode project, framework-link/Swift host, build settings, SwiftPM or signing command.
set +e
(cd "$APP" && ./gradlew :composeApp:compileKotlinIosSimulatorArm64 \
  --include-build "$ENGINE" --no-daemon --no-parallel --max-workers=1 \
  --no-build-cache --no-configuration-cache --console=plain --stacktrace \
  -Pkotlin.compiler.execution.strategy=in-process -PkiraUseMavenLocal=false \
  -Dorg.gradle.vfs.watch=false '-Dorg.gradle.jvmargs=-Xmx3g -XX:MaxMetaspaceSize=1g') > reports/compile.log 2>&1
compile_code=$?
stop_gradle compile
compile_stop_code=$?
set -e
printf 'command_exit=%s\nstop_exit=%s\n' "$compile_code" "$compile_stop_code" > reports/compile-status.txt
for component_code in "$selection_code" "$native_code" "$native_stop_code" "$native_verify_code" "$native_cleanup_code" \
  "$compile_code" "$compile_stop_code"; do
