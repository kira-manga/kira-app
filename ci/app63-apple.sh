...
swiftpm_negative_code=NOT_RUN; swiftpm_preflight_code=NOT_RUN; swiftpm_preflight_verify_code=NOT_RUN
swiftpm_host_check_code=NOT_RUN; swiftpm_host_verify_code=NOT_RUN
cleanup() {
  ...
  python3 control/ci/app63-apple-verify.py swiftpm-restore > reports/swiftpm-final-restore.log 2>&1; restore_code=$?
  ...
  printf 'swiftpm_final_restore_exit=%s\nswiftpm_negative_exit=%s\nswiftpm_preflight_exit=%s\nswiftpm_preflight_verification_exit=%s\nswiftpm_host_check_exit=%s\nswiftpm_host_verification_exit=%s\n' \
    "$restore_code" "$swiftpm_negative_code" "$swiftpm_preflight_code" "$swiftpm_preflight_verify_code" \
    "$swiftpm_host_check_code" "$swiftpm_host_verify_code" >> reports/result.txt
  ...
  if [ "$app_code" -eq 0 ] && [ "$engine_code" -eq 0 ] && [ "$stop_code" -eq 0 ] && [ "$cleanup_code" -eq 0 ] && [ "$restore_code" -eq 0 ]; then
...
(cd "$APP" && ./gradlew :platform:iosSimulatorArm64Test \
  ...
  --tests me.manga.kira.platform.download.IosBackgroundEventDrainTest \
  --tests me.manga.kira.platform.cbz.IosCbzRetainedInputTest \
  --tests me.manga.kira.platform.filesystem.IosAppFileSystemBackupTest \
  :data:iosSimulatorArm64Test \
  --tests me.manga.kira.data.repository.IosBackgroundArtifactTest \
  --tests me.manga.kira.data.repository.IosBackgroundEventSettlementTest \
  --tests me.manga.kira.data.repository.IosBackgroundFailedCleanupTest \
  --tests me.manga.kira.data.repository.IosCbzConversionTest \
  --tests me.manga.kira.data.repository.IosManifestPersistenceTest --continue \
...
# Use the real current App61 helper; no mocked Xcode, protected inputs, pin refresh, or second project.
if python3 control/ci/app63-apple-verify.py swiftpm-negatives > reports/swiftpm-negative.log 2>&1; then
  swiftpm_negative_code=0
else swiftpm_negative_code=$?; exit "$swiftpm_negative_code"; fi
if (cd "$APP" && /usr/bin/ruby -r ./scripts/release/verify-toolchain-inputs.rb \
    -e 'puts ToolchainInputVerifier::SWIFTPM_PREFLIGHT_COMMAND' > "$GITHUB_WORKSPACE/reports/swiftpm-preflight-command.sh" && \
    RUNNER_TEMP="$RUN" bash -e -o pipefail "$GITHUB_WORKSPACE/reports/swiftpm-preflight-command.sh") \
    > reports/swiftpm-preflight.log 2>&1; then
  swiftpm_preflight_code=0
else swiftpm_preflight_code=$?; fi
printf 'command_exit=%s\n' "$swiftpm_preflight_code" > reports/swiftpm-preflight-status.txt
if [ "$swiftpm_preflight_code" -ne 0 ]; then exit 1; fi
if python3 control/ci/app63-apple-verify.py swiftpm preflight > reports/swiftpm-preflight-verify.log 2>&1; then
  swiftpm_preflight_verify_code=0
else swiftpm_preflight_verify_code=$?; exit "$swiftpm_preflight_verify_code"; fi
# The same reviewed generated lock and per-run package directories feed this one unsigned host.
set +e
xcodebuild -project "$APP/iosApp/iosApp.xcodeproj" -scheme iosApp \
  -configuration Debug -sdk iphonesimulator -destination 'generic/platform=iOS Simulator' \
  -derivedDataPath "$RUN/DerivedData" -clonedSourcePackagesDirPath "$RUN/DerivedData/SourcePackages" \
  -packageCachePath "$RUN/kira-swiftpm-cache" \
  -onlyUsePackageVersionsFromResolvedFile -disableAutomaticPackageResolution -skipPackageUpdates \
  -jobs 1 ARCHS=arm64 ONLY_ACTIVE_ARCH=YES \
...
for config in Debug Release; do
  if xcodebuild -project "$APP/iosApp/iosApp.xcodeproj" -scheme iosApp -configuration "$config" \
    ...
    -derivedDataPath "$RUN/DerivedData" -clonedSourcePackagesDirPath "$RUN/DerivedData/SourcePackages" \
    -packageCachePath "$RUN/kira-swiftpm-cache" \
    -onlyUsePackageVersionsFromResolvedFile -disableAutomaticPackageResolution -skipPackageUpdates -showBuildSettings -json \
...
set +e
(cd "$APP" && /usr/bin/ruby scripts/release/verify-toolchain-inputs.rb --check-swiftpm) > reports/swiftpm-host-check.log 2>&1
swiftpm_host_check_code=$?
python3 control/ci/app63-apple-verify.py swiftpm host > reports/swiftpm-host-verify.log 2>&1
swiftpm_host_verify_code=$?
set -e
...
for component_code in "$selection_code" "$native_code" "$native_stop_code" "$native_verify_code" "$native_cleanup_code" \
  "$host_code" "$host_stop_code" "$host_verify_code" "$settings_debug_code" "$settings_release_code" "$settings_verify_code" \
  "$swiftpm_negative_code" "$swiftpm_preflight_code" "$swiftpm_preflight_verify_code" \
  "$swiftpm_host_check_code" "$swiftpm_host_verify_code"; do
