"""App13 compile-only evidence; fixed source/Native boundaries, no replay or tool launch."""
import hashlib
import json
import os
...
def environment(run, xcode, control):
    ...
    env = dict(PATH=java + '/bin:/usr/bin:/bin:/usr/sbin:/sbin', JAVA_HOME=java, HOME=str(run / 'home'),
        ...
        KIRA_APP13_RUN=str(run), KIRA_APP13_CONTROL=str(control),
        JAVA_OPTS=f'-Xmx128m -Dapp13.apple.owner={run.name} -Duser.home={run / "home"} -Djava.io.tmpdir={run / "tmp"}')
