 ...
 */
package me.manga.kira.sources_repositry.tr.webtoontr
...
class WebtoontrRepository(
    ...
    override suspend fun initSite(): Int {
        ...
        Logger.withTag("WebtoontrInit").i { "Headers from DataStore: $headers" }
        _cachedHeaders = headers
        Logger.withTag("WebtoontrInit").i { "_cachedHeaders is now: $_cachedHeaders" }
    ...
    override suspend fun extractMangaInfo(
        ...
        val description = doc.selectFirst("div.manga-excerpt.summary__content.show-more p")
            ?.text()?.trim().orEmpty()
        // Alternative names
        ...
        val postContent2 = summaryContent?.selectFirst("div.post-content")

        // Genres - FIXED: Use "Tür(ler)" which means "Genre(s)" in Turkish
        val genres = postContent?.select(".post-content_item:has(.summary-heading:contains(Tür)) .summary-content .genres-content a")
            ?.map { it.text() }?.toMutableList() ?: mutableListOf()

        // Type (Manhwa, Manga, Manhua, etc.) - Fixed selector
        val type = postContent2?.selectFirst(".post-content_item:has(.summary-heading:contains(Cinsi)) .summary-content")
            ?.text()?.trim()
...
//            .distinct()
        Logger.withTag("jkafhdjkfdasjfdskfjsdfsfssafdfds").i { imgs.toString() }
