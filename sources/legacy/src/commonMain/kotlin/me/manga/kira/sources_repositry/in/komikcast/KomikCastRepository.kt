 ...
 */
package me.manga.kira.sources_repositry.`in`.komikcast
    ...
    override suspend fun extractMangaInfo(
        ...
        val chapters = doc.select("div.komik_info-chapters li").map { el ->
            ...
            val parsed = try {
                ...
                when {
                    rawDate.contains("hour", true) -> today
                    rawDate.contains("day", true) -> today.minus(num.toInt(), DateTimeUnit.DAY)
                    rawDate.contains("week", true) -> today.minus(num.toInt(), DateTimeUnit.WEEK)
                    rawDate.contains("month", true) -> today.minus(num.toInt(), DateTimeUnit.MONTH)
                    rawDate.contains("year", true) -> today.minus(num.toInt(), DateTimeUnit.YEAR)
                    else -> today
                }
            } catch (e: Exception) {
