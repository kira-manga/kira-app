package me.manga.kira.sources_repositry.it.mangaworld
 ...
 *
 * Italian month-name parsing in [parseItalianDateText] is deferred — kotlinx.datetime cannot
 * resolve locale-aware text months. The function now returns null on the Italian-month branch,
 * and call sites fall back to "today" so the chapter date chain still works. See
 * TODO(Phase 8 - locale) below.
 */
    ...
    override fun extractCustomHomeMangaItems(html: String): MutableList<MangaItem> {
        ...
        return doc.select("div.comics-grid div.entry").mapNotNull { entry ->
            try {
                ...
                Logger.withTag("MangaworldIt").e { "Error parsing manga entry: ${e.message ?: "unknown"}" }
                null
            }
        }.toMutableList()
    }
...
// First: try MangaWorld layout (no volumes)
    ...
    //
    // The original Android implementation looked up Italian month names (Gennaio, Febbraio, ...)
    // and produced a `java.time.LocalDate`. kotlinx.datetime's locale-aware month-name parsing is
    // not available in commonMain — `MonthNames` only ships an English locale — so the
    // Italian-name branch returns null here and lets callers fall back to "today". The numeric
    // regex match is preserved so the source's behaviour for non-textual months would still work
    // if the upstream HTML ever uses numbers, but in practice MangaWorld serves Italian names so
    // the current effective behaviour is "null → fallback to today" (same as the Android catch
    // branch's `LocalDate.now()`).
    //
    // TODO(Phase 8 - locale): replace with a locale-aware parser (expect/actual ICU bindings or
    //   a custom MonthNames for Italian) so Italian chapter dates are parsed accurately again.
    private fun parseItalianDateText(dateText: String): LocalDate? {
        ...
        val month = months[capitalized] ?: return null
        return try {
    ...
    override fun getChapterImages(html: String): List<String> {
        ...
        try {
            ...
            Logger.withTag("ChapterImages").e(e) { "Error parsing chapter data" }
        }
