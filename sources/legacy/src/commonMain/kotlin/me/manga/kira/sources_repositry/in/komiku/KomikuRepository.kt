 ...
 *
 * Additional non-mechanical port detail:
 *  - `okhttp3.HttpUrl.toHttpUrl().newBuilder().addQueryParameter(...)` (URL builder for the search
 *    URL) replaced by a manual percent-encoded query-string concatenation, since OkHttp's HttpUrl
 *    is JVM-only. Behaviour is preserved (single `s=<query>` parameter appended onto the path).
 */
package me.manga.kira.sources_repositry.`in`.komiku
...
class KomikuRepository(
    ...
    override fun handelSearchUrl(searchType: SearchType): String {
        // Source used okhttp3 HttpUrl builder to append `s=<query>` to
        // "$baseUrlApi/page/1/?post_type=manga". Manual concat preserves behaviour while staying KMP.
        val q = searchType.toNormalQuery().encodeURLParameter()
        return "$baseUrlApi/page/1/?post_type=manga&s=$q"
    }
    ...
    fun extractKomikuMangaItems(html: String): List<MangaItem> {
        val doc = Ksoup.parse(html, BASE_URL)   // IMPORTANT FIX
    ...
    // dd/MM/yyyy formatter — replaces source's java.time.format.DateTimeFormatter.ofPattern.
    ...
    override suspend fun extractMangaInfo(
        ...
        Logger.withTag("kjhjkhkjhjhjlkhjkhkjhl").i { baseUrl }
        ...
        for (i in 1 until chapterRows.size) {
            ...
            val localDate = runCatching {
                LocalDate.parse(dateText, slashDateFormatter)
