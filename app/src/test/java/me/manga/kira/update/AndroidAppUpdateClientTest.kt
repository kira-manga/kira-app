...
class AndroidAppUpdateClientTest {
    ...
    @Test
    fun selectionAndLaunchAgreeForEverySupportedAvailabilityRow() =
        runTest(dispatcher) {
    ...
    @Test
    fun startRevalidatesFreshStateAndPreservesSdkFailureAndCancellation() =
        runTest(dispatcher) {
    ...
    @Test
    fun launchResolvesTheCurrentUsableHostOnlyAfterTheSdkQuery() =
        runTest(dispatcher) {
    ...
    @Test
    fun recoveryResumesImmediateButNeverPromotesAFlexibleDownload() =
        runTest(dispatcher) {
    ...
    @Test
    fun downloadedListenerIsReplacedAndUnregisteredIdempotently() =
        runTest(dispatcher) {
            val manager = newManager().apply { fake.setUpdateAvailable(VERSION, AppUpdateType.FLEXIBLE) }
            val client = client(manager)
    ...
    private fun client(manager: RecordingAppUpdateManager) = AndroidAppUpdateClient(application, { activity }, manager)
