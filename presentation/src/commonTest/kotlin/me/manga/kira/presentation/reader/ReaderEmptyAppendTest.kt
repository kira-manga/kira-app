...
class ReaderEmptyAppendTest {
    ...
    @Test
    fun emptyMiddleBoundaryNamesTheChapterThatTheNextAppendFetches() = withReader { env ->
        ...
        assertEquals(listOf("page/1", "page/3"), env.vm.state.value.pages.map { it.url })
        assertNull(env.vm.state.value.boundary().nextChapter)
        assertEquals(chapters[0].url, env.vm.state.value.activeChapterUrl)
    }

    @Test
    fun terminalEmptyIsNotRetriedAndExplicitReplacementResetsTheOutcome() = withReader(chapters.take(2)) { env ->
        env.pages.result = flowOf(AppResult.Success(emptyList()))
        env.vm.submit(ReaderIntent.OnAppendNextChapter)
        repeat(3) { env.vm.submit(ReaderIntent.OnAppendNextChapter) }
        assertNull(env.vm.state.value.boundary().nextChapter)
        ...
        assertTrue(env.vm.state.value.skippedChapterUrls.isEmpty())
        assertEquals(listOf(chapters[1].url), env.vm.state.value.loadedChapterUrls)
        assertEquals(listOf("page/2"), env.vm.state.value.pages.map { it.url })
    }

    @Test
    fun streamingRecoveryClearsSkipWithoutLosingPagesOnLaterEmptyEmissions() = withReader { env ->
        val stream = MutableSharedFlow<AppResult<List<Page>>>()
        env.pages.result = stream
        env.vm.submit(ReaderIntent.OnAppendNextChapter)
        stream.emit(AppResult.Success(emptyList()))
        assertEquals(chapters[2], env.vm.state.value.boundary().nextChapter)
        repeat(3) { env.vm.submit(ReaderIntent.OnAppendNextChapter) }
        ...
        stream.emit(AppResult.Success(emptyList()))
        assertTrue(env.vm.state.value.skippedChapterUrls.isEmpty())
        assertEquals(listOf("page/1", "page/2a"), env.vm.state.value.pages.map { it.url })
        stream.emit(AppResult.Success(listOf(readerPage("page/2a"), readerPage("page/2b"))))
        assertEquals(listOf("page/1", "page/2a", "page/2b"), env.vm.state.value.pages.map { it.url })
    }
