...
class IosCbzNativeOwnershipTest {
    @Test
    fun eachNativeBandIsCopiedFreedAndEmittedBeforeTheNextEncodeStarts() = runTest {
        val native = IosCbzRecordingCodec()
        val inspector = IosPageMediaInspector()
        var emitted = 0
        val result = IosLibWebpEncoder.encodeValidatedPage(
            IOS_CBZ_PNG, inspector.inspect(IOS_CBZ_PNG).requireValid(), 75, 24, CbzWriter.DEFAULT_MAX_MEMORY_BYTES, native,
        ) { bytes ->
    ...
    @Test
    fun nativeOutputSizeIsCheckedBeforeCopyAndFreedEvenWhenCopyingFails() = runTest {
        NativeOutputFault.entries.forEach { fault ->
            val native = object : IosCbzRecordingCodec() {
        ...
        iosCbzTest { fixture ->
            listOf(false, true).forEachIndexed { index, contextFailure ->
                ...
                val native = object : IosCbzRecordingCodec() {
                    ...
                    override fun createContext(plan: CbzTranscodePlan, colorSpace: CGColorSpaceRef): CGContextRef? = null
                }
    ...
    fun rollbackEncoderLateEmissionFailureAlsoRetainsThePreviousArchive() =
        iosCbzTest { fixture ->
            ...
            val originals = fixture.capture(paths)
            var written = 0
            val encoder = IosCbzPageEncoder { source, metadata, quality, maxHeight, maxMemoryBytes, emit ->
