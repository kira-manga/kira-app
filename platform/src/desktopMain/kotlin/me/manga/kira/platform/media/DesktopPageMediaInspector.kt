...
class DesktopPageMediaInspector(
    ...
    private fun inspectSnapshot(data: Data): PageInspection =
        inspectPageInput(policy, data.size.toLong(), { DesktopPageDataSource(data) }) { format ->
            if (format == PageImageFormat.AVIF) PageInspection.Rejected(PageInspectionRejection.DECODER_UNAVAILABLE)
            else inspectCodec(data, format)
        }

    private fun inspectCodec(data: Data, expected: PageImageFormat): PageInspection = try {
...
private fun PageImageFormat.skiaFormat(): EncodedImageFormat? = when (this) {
