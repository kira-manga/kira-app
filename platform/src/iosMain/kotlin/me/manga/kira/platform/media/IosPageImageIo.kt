...
private fun imageIoMetadata(
    ...
    return try {
        PageImageMetadata(
            format,
            imageDimension(properties, kCGImagePropertyPixelWidth),
            imageDimension(properties, kCGImagePropertyPixelHeight),
        )
    } finally {
...
@OptIn(ExperimentalForeignApi::class)
private fun validateImageIoSample(
    source: CGImageSourceRef,
    edge: Int,
): Boolean =
    memScoped {
        ...
        try {
            ...
            try {
                ...
                width in 1uL..edge.toULong() &&
                    height in 1uL..edge.toULong() &&
                    row > 0uL &&
                    row <= MAX_NATIVE_SAMPLE_BYTES / height
            } finally {
...
@OptIn(ExperimentalForeignApi::class)
private fun imageIoFormat(type: CFStringRef?): PageImageFormat? =
    memScoped {
